...
const reportDir = path.join(root, "reports", "restricted-mode-final-release");
const reportFile = path.join(reportDir, "restricted-mode-final-release-status.json");

const publicJsonFile = path.join(root, "public-docs", "restricted-mode-final-release-status.json");
const publicHtmlFile = path.join(root, "public-docs", "restricted-mode-final-release.html");

fs.mkdirSync(reportDir, { recursive: true });

const releaseCandidateArchivePath = "release/astra-treasury-protocol-v0.1.1-restricted-mode-release-candidate.tar.gz";

const finalReleaseArtifactPaths = [
  releaseCandidateArchivePath,
  ...
  "reports/action-approvals/governance-decision/action-approval-record.json",
  "reports/signed-governance-resolution-evidence/import/signed-governance-resolution-evidence.json",
  "reports/restricted-mode-evidence-seal/restricted-mode-evidence-seal.json",
  "reports/restricted-mode-release-candidate/restricted-mode-release-candidate-status.json"
];
...
const config = readJson("configs/restricted-mode-final-release.config.json");
...
check(checks, "Final release config prepared", config.finalReleasePrepared === true, {
  finalReleasePrepared: config.finalReleasePrepared
});
...
check(checks, "Evidence seal generated", evidenceSeal.status === "RESTRICTED_MODE_EVIDENCE_SEALED_DECISION_RECORDED_ALL_DISABLED", {
  ...
  sealHash: evidenceSeal.seal?.sealHash || ""
});
...
check(checks, "Release candidate ready", releaseCandidate.status === "RESTRICTED_MODE_RELEASE_CANDIDATE_READY_DECISION_RECORDED_ALL_DISABLED", {
  ...
  releaseCandidateHash: releaseCandidate.releaseCandidate?.releaseCandidateHash || ""
});

check(checks, "Release candidate archive exists", fs.existsSync(path.join(root, releaseCandidateArchivePath)), {
  releaseCandidateArchivePath
});
...
const missingArtifacts = finalReleaseArtifactPaths.filter((artifactPath) => {
  return !fs.existsSync(path.join(root, artifactPath));
});

for (const artifactPath of missingArtifacts) {
  check(checks, `Final release artifact exists: ${artifactPath}`, false, { artifactPath });
}

const artifactHashes = finalReleaseArtifactPaths
  .filter((artifactPath) => fs.existsSync(path.join(root, artifactPath)))
  .map((artifactPath) => sha256File(artifactPath));

const finalReleasePayload = {
  schema: "astra-restricted-mode-final-release-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  finalReleaseScope: "post-governance-decision-restricted-mode-all-disabled",
  artifactHashes
};

const finalReleaseHash = sha256Json(finalReleasePayload);
const failures = checks.filter((item) => !item.pass);

const status = failures.length === 0
  ? "RESTRICTED_MODE_FINAL_RELEASE_READY_DECISION_RECORDED_ALL_DISABLED"
  : "RESTRICTED_MODE_FINAL_RELEASE_REVIEW_REQUIRED";

const releaseCandidateArchive = fs.existsSync(path.join(root, releaseCandidateArchivePath))
  ? sha256File(releaseCandidateArchivePath)
  : {
      path: releaseCandidateArchivePath,
      bytes: 0,
      sha256: ""
    };

const report = {
  schema: "astra-restricted-mode-final-release-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  finalReleaseScope: config.finalReleaseScope || "post-governance-decision-restricted-mode-all-disabled",
  publicStatement:
    "AstraTreasury restricted-mode final release is ready. Governance decision is recorded, all restricted capabilities remain disabled and not approved, and release evidence is sealed.",
  summary: {
...
<head>
  ...
  <title>AstraTreasury Restricted-Mode Final Release</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Final release · restricted mode</div>
    <h1>Restricted-Mode Final Release</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Final release hash:</strong> <code>${escapeHtml(finalReleaseHash)}</code></p>
    <p><strong>Release candidate archive:</strong> <code>${escapeHtml(releaseCandidateArchive.sha256)}</code></p>
  </section>

  <section class="card">
    <h2>Final release artifacts</h2>
    <table>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This final release is restricted-mode only. It does not approve launch, funding,
      public sale, staking/rewards, buybacks, execution, automation, autonomous execution,
      Safe payload generation, or Safe transaction execution.
    </div>
  </section>

  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/restricted-mode-final-release">/api/public/restricted-mode-final-release</a></p>
    <p><a href="/restricted-mode-release-candidate">Restricted-Mode Release Candidate</a></p>
    <p><a href="/restricted-mode-evidence-seal">Restricted-Mode Evidence Seal</a></p>
...
console.log("AstraTreasury Restricted-Mode Final Release");
console.log("===========================================");
console.log(`Status: ${report.status}`);
console.log(`Final release artifacts: ${report.summary.artifactCount}`);
console.log(`Final release hash: ${finalReleaseHash}`);
console.log(`Release candidate archive hash: ${releaseCandidateArchive.sha256}`);
