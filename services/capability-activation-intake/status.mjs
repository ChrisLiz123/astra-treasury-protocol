...
const reportDir = path.join(root, "reports", "capability-activation-intake");
const reportFile = path.join(reportDir, "capability-activation-intake-status.json");

const publicJsonFile = path.join(root, "public-docs", "capability-activation-intake-status.json");
const publicHtmlFile = path.join(root, "public-docs", "capability-activation-intake.html");

fs.mkdirSync(reportDir, { recursive: true });

const intakeArtifactPaths = [
  ...
  if (request) {
    ...
    for (const key of [
      ...
      if (request[key] !== false) {
        requestIssues.push(`${key} must be false for intake.`);
      }
    }
...
const missingArtifacts = intakeArtifactPaths.filter((artifactPath) => {
  return !fs.existsSync(path.join(root, artifactPath));
});

for (const artifactPath of missingArtifacts) {
  check(checks, `Intake artifact exists: ${artifactPath}`, false, { artifactPath });
}

const artifactHashes = intakeArtifactPaths
...
const intakePayload = {
  schema: "astra-capability-activation-intake-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  intakeScope: "future-capability-request-intake-only",
  requestFilePresent: fs.existsSync(requestPath),
  requestValid,
  artifactHashes
};

const intakeGateHash = sha256Json(intakePayload);
const failures = checks.filter((item) => !item.pass);

const status = failures.length > 0
  ? "CAPABILITY_ACTIVATION_INTAKE_GATE_REVIEW_REQUIRED"
  : requestValid
...
const report = {
  schema: "astra-capability-activation-intake-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  intakeScope: config.intakeScope || "future-capability-request-intake-only",
  publicStatement:
    requestValid
      ? "AstraTreasury capability activation request has been imported for review. Intake does not approve activation or generate any Safe payload."
      : "AstraTreasury capability activation intake gate is ready. No active capability activation request is imported, and all restricted capabilities remain disabled and not approved.",
  summary: {
    requestFilePresent: fs.existsSync(requestPath),
    requestValid,
...
const allowedRows = (config.allowedCapabilities || []).map((item) => {
  return `<tr><td>${escapeHtml(item)}</td><td>Requires separate action-specific approval</td></tr>`;
}).join("");
...
<head>
  ...
  <title>AstraTreasury Capability Activation Intake</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Intake gate · no capability approved</div>
    <h1>Capability Activation Intake</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Intake gate hash:</strong> <code>${escapeHtml(intakeGateHash)}</code></p>
  </section>

  <section class="card">
    <h2>Allowed intake capabilities</h2>
    <table>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This intake gate accepts requests for review only. It does not approve launch, funding,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/capability-activation-intake">/api/public/capability-activation-intake</a></p>
...
console.log("AstraTreasury Capability Activation Intake Gate");
console.log("===============================================");
...
console.log(`Intake gate hash: ${intakeGateHash}`);
