...
const reportDir = path.join(root, "reports", "restricted-mode-operations-handoff");
const reportFile = path.join(reportDir, "restricted-mode-operations-handoff-status.json");

const publicJsonFile = path.join(root, "public-docs", "restricted-mode-operations-handoff-status.json");
const publicHtmlFile = path.join(root, "public-docs", "restricted-mode-operations-handoff.html");
...
const handoffArtifactPaths = [
  ...
  "docs/restricted-mode-operations-handoff/RESTRICTED_MODE_OPERATIONS_HANDOFF.md",
  "docs/restricted-mode-operations-handoff/STEADY_STATE_OPERATIONS_RUNBOOK.md",
  "docs/restricted-mode-operations-handoff/CAPABILITY_ACTIVATION_BOUNDARIES.md",
  "docs/restricted-mode-operations-handoff/INCIDENT_RESPONSE_HANDOFF.md"
];
...
const config = readJson("configs/restricted-mode-operations-handoff.config.json");
...
const missingArtifacts = handoffArtifactPaths.filter((artifactPath) => {
  return !fs.existsSync(path.join(root, artifactPath));
});

for (const artifactPath of missingArtifacts) {
  check(checks, `Handoff artifact exists: ${artifactPath}`, false, { artifactPath });
}

const artifactHashes = handoffArtifactPaths
  .filter((artifactPath) => fs.existsSync(path.join(root, artifactPath)))
  .map((artifactPath) => sha256File(artifactPath));

const handoffPayload = {
  schema: "astra-restricted-mode-operations-handoff-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  handoffScope: "post-final-release-restricted-mode-all-disabled",
  artifactHashes
};

const handoffHash = sha256Json(handoffPayload);
const failures = checks.filter((item) => !item.pass);

const status = failures.length === 0
  ? "RESTRICTED_MODE_OPERATIONS_HANDOFF_READY_DECISION_RECORDED_ALL_DISABLED"
  : "RESTRICTED_MODE_OPERATIONS_HANDOFF_REVIEW_REQUIRED";

const report = {
  schema: "astra-restricted-mode-operations-handoff-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  handoffScope: config.handoffScope || "post-final-release-restricted-mode-all-disabled",
  publicStatement:
    "AstraTreasury restricted-mode operations handoff is ready. Governance decision is recorded, final restricted-mode release is ready, and all restricted capabilities remain disabled and not approved.",
  summary: {
  ...
  steadyStateOperations: config.steadyStateOperations || {},
  checks,
  failures,
  missingArtifacts,
  currentStatuses: {
...
const operationsRows = Object.entries(report.steadyStateOperations).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury Restricted-Mode Operations Handoff</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Operations handoff · restricted mode</div>
    <h1>Restricted-Mode Operations Handoff</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Handoff hash:</strong> <code>${escapeHtml(handoffHash)}</code></p>
  </section>

  <section class="card">
    <h2>Steady-state operations</h2>
    <table>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This operations handoff documents restricted-mode steady-state operations only.
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/restricted-mode-operations-handoff">/api/public/restricted-mode-operations-handoff</a></p>
...
console.log("AstraTreasury Restricted-Mode Operations Handoff");
console.log("================================================");
console.log(`Status: ${report.status}`);
console.log(`Handoff artifacts: ${report.summary.artifactCount}`);
console.log(`Handoff hash: ${handoffHash}`);
