...
const reportDir = path.join(root, "reports", "dex-liquidity-funding-transfer-payload-generation-approval");
const reportFile = path.join(reportDir, "dex-liquidity-funding-transfer-payload-generation-approval-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-payload-generation-approval-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-payload-generation-approval.html");
...
const approvalRecordPresent = Boolean(record && record.schema === "astra-dex-liquidity-funding-transfer-payload-generation-approval-record-v0.1");
...
check("Funding transfer approval recorded", transferApproval.status === "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_APPROVED_NO_PAYLOAD_NO_FUNDS_MOVED", {
  status: transferApproval.status || "UNKNOWN"
});

check("No transfer payload or funds movement", transferApproval.summary?.fundingTransferPayloadGenerated === false && transferApproval.summary?.fundingTransferExecuted === false && transferApproval.summary?.treasuryFundsMoved === false, {
  fundingTransferPayloadGenerated: transferApproval.summary?.fundingTransferPayloadGenerated,
  fundingTransferExecuted: transferApproval.summary?.fundingTransferExecuted,
  treasuryFundsMoved: transferApproval.summary?.treasuryFundsMoved
});
...
check("Funding source and destination ready", requirementsStatus.summary?.sourceAddressProvided === true && requirementsStatus.summary?.sourceAddressRequiredBeforePayloadGeneration === false, {
...
const approvalPayload = {
  schema: "astra-dex-liquidity-funding-transfer-payload-generation-approval-public-payload-v0.1",
  approvalRecordPresent,
  fundingTransferPayloadGenerationApproved: approvalRecordPresent,
...
const status = failures.length > 0
  ? "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_PAYLOAD_GENERATION_APPROVAL_REVIEW_REQUIRED"
  : approvalRecordPresent
    ? "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_PAYLOAD_GENERATION_APPROVED_NO_PAYLOAD_NO_FUNDS_MOVED"
    : "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_PAYLOAD_GENERATION_APPROVAL_READY_PENDING_APPROVAL";
...
const report = {
  schema: "astra-dex-liquidity-funding-transfer-payload-generation-approval-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: approvalRecordPresent
    ? "AstraTreasury approved generation of the DEX liquidity funding-transfer Safe payload in a later step. No payload has been generated, no treasury funds have moved, no token approvals have been executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity funding-transfer payload generation approval framework is ready. Approval has not yet been recorded.",
  summary: {
    approvalRecordPresent,
    fundingTransferPayloadGenerationApproved: approvalRecordPresent,
    ...
    publicTradingApproved: false,
    fullLaunchApproved: false,
    nextRecommendedMilestone: "DEX Liquidity Treasury Funding Transfer Safe Payload Generation",
  ...
  fundingTransferPayloadGenerationApproval: {
    approvalHash,
  ...
  missingArtifacts,
  currentStatuses: {
    dexLiquidityFundingTransferApproval: transferApproval.status || "UNKNOWN",
...
const transferRows = (report.fundingTransferPayloadGenerationApproval.tokenTransferRequirements || []).map((item) => {
  return `<tr>
...
const requiredRows = Object.entries(report.fundingTransferPayloadGenerationApproval.requiredBeforeFundingTransferPayloadGeneration).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Funding Transfer Payload Generation Approval</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Funding-transfer payload generation approval · no payload · no funds moved</div>
    <h1>DEX Funding Transfer Payload Generation Approval</h1>
  ...
  <section class="card">
    <h2>Required before payload generation</h2>
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-funding-transfer-payload-generation-approval">/api/public/dex-liquidity-funding-transfer-payload-generation-approval</a></p>
    <p><a href="/dex-liquidity-funding-transfer-approval">DEX Funding Transfer Approval</a></p>
...
console.log("AstraTreasury DEX Funding Transfer Payload Generation Approval");
console.log("==============================================================");
console.log(`Status: ${report.status}`);
console.log(`Approval record present: ${approvalRecordPresent}`);
console.log(`Payload generation approved: ${report.summary.fundingTransferPayloadGenerationApproved}`);
console.log(`Payload generated: ${report.summary.fundingTransferPayloadGenerated}`);
