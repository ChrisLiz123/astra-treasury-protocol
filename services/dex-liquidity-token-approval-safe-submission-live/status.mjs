...
const reportDir = path.join(root, "reports", "dex-liquidity-token-approval-safe-submission-live");
const statusReportFile = path.join(reportDir, "dex-liquidity-token-approval-safe-submission-live-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-token-approval-safe-submission-live-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-token-approval-safe-submission-live.html");
...
const recordPresent = Boolean(record && record.schema === "astra-dex-liquidity-token-approval-safe-submission-live-record-v0.1");
...
const status = recordPresent
  ? record.status
  : "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_SUBMISSION_LIVE_READY_PENDING_SUBMISSION_EVIDENCE";
...
const submissionPayload = {
  schema: "astra-dex-liquidity-token-approval-safe-submission-live-public-payload-v0.1",
  recordPresent,
  safeTxHash: record?.safeTxHash || "",
  safeNonce: record?.safeNonce ?? "",
  payloadHash: record?.payloadHash || "",
  tokenApprovalSafeTransactionSubmitted: recordPresent,
...
const publicPayload = {
  schema: "astra-dex-liquidity-token-approval-safe-submission-live-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: recordPresent
    ? "AstraTreasury recorded live Safe submission/proposal evidence for the DEX liquidity token-approval payload. The Safe transaction is not executed, no token approval has executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity token-approval Safe submission live framework is ready. Submission evidence has not yet been recorded.",
  summary: {
    recordPresent,
    tokenApprovalSafeSubmissionLiveRecorded: recordPresent,
    tokenApprovalSafeTransactionSubmitted: recordPresent,
    ...
    publicTradingApproved: false,
    fullLaunchApproved: false,
    nextRecommendedMilestone: "DEX Liquidity Token Approval Safe Pending Signatures Monitoring",
    artifactCount: artifactHashes.length
  },
  tokenApprovalSafeSubmissionLive: {
    submissionHash,
    hashAlgorithm: "SHA-256",
    record: record || {},
    requiredBeforeTokenApprovalPendingSignaturesMonitoring: record?.requiredBeforeTokenApprovalPendingSignaturesMonitoring || {},
    artifactHashes
  },
  currentStatuses: {
...
const requiredRows = Object.entries(publicPayload.tokenApprovalSafeSubmissionLive.requiredBeforeTokenApprovalPendingSignaturesMonitoring || {}).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Token Approval Safe Submission Live</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Safe submission live · not executed · no approvals executed</div>
    <h1>DEX Token Approval Safe Submission Live</h1>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This records proposal/submission only. The Safe transaction is not executed, token approvals have not executed,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-token-approval-safe-submission-live">/api/public/dex-liquidity-token-approval-safe-submission-live</a></p>
...
console.log("AstraTreasury DEX Token Approval Safe Submission Live");
console.log("=====================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Submission recorded: ${publicPayload.summary.tokenApprovalSafeSubmissionLiveRecorded}`);
