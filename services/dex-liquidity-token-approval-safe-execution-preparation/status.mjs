...
const reportDir = path.join(root, "reports", "dex-liquidity-token-approval-safe-execution-preparation");
const statusReportFile = path.join(reportDir, "dex-liquidity-token-approval-safe-execution-preparation-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-token-approval-safe-execution-preparation-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-token-approval-safe-execution-preparation.html");
...
const recordPresent = Boolean(record && record.schema === "astra-dex-liquidity-token-approval-safe-execution-preparation-v0.1");
...
const status = recordPresent
  ? record.status
  : "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_EXECUTION_PREPARATION_NOT_RUN";
...
const publicPayload = {
  schema: "astra-dex-liquidity-token-approval-safe-execution-preparation-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: status === "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_EXECUTION_PREPARED_NOT_EXECUTED_NO_APPROVALS_EXECUTED_NO_LIQUIDITY"
    ? "AstraTreasury prepared the DEX liquidity token-approval Safe execution step. The Safe transaction has not been executed, token approvals have not executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity token-approval Safe execution preparation has not completed successfully.",
  summary: {
    recordPresent,
    tokenApprovalSafeExecutionPreparationComplete: status === "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_EXECUTION_PREPARED_NOT_EXECUTED_NO_APPROVALS_EXECUTED_NO_LIQUIDITY",
    ...
    nextRecommendedMilestone: "DEX Liquidity Token Approval Safe Execution Live",
    artifactCount: artifactHashes.length
  },
  tokenApprovalSafeExecutionPreparation: {
    ...
    requiredBeforeTokenApprovalSafeExecutionLive: record?.requiredBeforeTokenApprovalSafeExecutionLive || {},
    artifactHashes
  },
  currentStatuses: {
    dexTokenApprovalSafeExecutionApproval: approvalStatus.status || "UNKNOWN",
...
const requiredRows = Object.entries(publicPayload.tokenApprovalSafeExecutionPreparation.requiredBeforeTokenApprovalSafeExecutionLive || {}).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Token Approval Safe Execution Preparation</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Safe execution prepared · not executed · no approvals executed</div>
    <h1>DEX Token Approval Safe Execution Preparation</h1>
    <p>${escapeHtml(publicPayload.publicStatement)}</p>
    <p><strong>Preparation hash:</strong> <code>${escapeHtml(publicPayload.tokenApprovalSafeExecutionPreparation.preparationHash)}</code></p>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This is preparation only. It has not executed the Safe transaction, token approvals have not executed,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-token-approval-safe-execution-preparation">/api/public/dex-liquidity-token-approval-safe-execution-preparation</a></p>
    <p><a href="/dex-liquidity-token-approval-safe-execution-approval">DEX Token Approval Safe Execution Approval</a></p>
...
console.log("AstraTreasury DEX Token Approval Safe Execution Preparation");
console.log("===========================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Execution preparation complete: ${publicPayload.summary.tokenApprovalSafeExecutionPreparationComplete}`);
console.log(`Service threshold reached: ${publicPayload.summary.serviceThresholdReached}`);
console.log(`Safe transaction executed: ${publicPayload.summary.tokenApprovalSafeTransactionExecuted}`);
