...
const statusReportFile = path.join(reportDir, "dex-liquidity-token-approval-post-execution-allowances-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-token-approval-post-execution-allowances-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-token-approval-post-execution-allowances.html");
...
const reportPresent = Boolean(report && report.schema === "astra-dex-liquidity-token-approval-post-execution-allowances-v0.1");
...
const status = reportPresent
  ? report.status
  : "DEX_LIQUIDITY_TOKEN_APPROVAL_POST_EXECUTION_ALLOWANCE_VERIFICATION_NOT_RUN";
...
const allowanceRows = (report?.allowanceVerificationChecks || []).map((check) => {
  return `<tr>
    <td>${escapeHtml(check.role)}</td>
    <td>${escapeHtml(check.symbol)}</td>
    <td><code>${escapeHtml(check.tokenAddress)}</code></td>
    <td>${escapeHtml(check.expectedAllowanceHuman || check.expectedAllowanceRaw)}</td>
    <td>${escapeHtml(check.allowanceEqualsExpected)}</td>
    <td>${escapeHtml(check.balanceCoversExpected)}</td>
  </tr>`;
}).join("");

const publicPayload = {
  schema: "astra-dex-liquidity-token-approval-post-execution-allowances-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: reportPresent
    ? "AstraTreasury verified post-execution DEX liquidity token approval allowances. Required allowances are available. Liquidity calldata has not been generated, liquidity has not been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity token approval post-execution allowance verification has not completed successfully.",
  summary: {
    reportPresent,
    tokenApprovalPostExecutionAllowanceVerificationComplete: report?.tokenApprovalPostExecutionAllowanceVerificationComplete === true,
    ...
    tokenApprovalSafeTransactionExecuted: report?.tokenApprovalSafeTransactionExecuted === true,
    tokenApprovalExecuted: report?.tokenApprovalExecuted === true,
    ...
    nextRecommendedMilestone: "DEX Liquidity Mint Calldata Generation Approval",
    artifactCount: artifactHashes.length
  },
  postExecutionAllowanceVerification: {
    verificationHash: report?.verificationHash || "",
    hashAlgorithm: "SHA-256",
    allowanceVerificationChecks: report?.allowanceVerificationChecks || [],
    requiredBeforeLiquidityMintCalldataGenerationApproval: report?.requiredBeforeLiquidityMintCalldataGenerationApproval || {},
    artifactHashes
  },
  currentStatuses: {
    dexTokenApprovalSafeExecutionLive: executionLiveStatus.status || "UNKNOWN",
  ...
  restrictions: {
    liquidityCalldataGeneration: false,
...
const requiredRows = Object.entries(publicPayload.postExecutionAllowanceVerification.requiredBeforeLiquidityMintCalldataGenerationApproval || {}).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Token Approval Post-Execution Allowances</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Allowances verified · approvals executed · no liquidity</div>
    <h1>DEX Token Approval Post-Execution Allowances</h1>
    <p>${escapeHtml(publicPayload.publicStatement)}</p>
    <p><strong>Verification hash:</strong> <code>${escapeHtml(publicPayload.postExecutionAllowanceVerification.verificationHash)}</code></p>
  </section>
  ...
  <section class="card">
    <h2>Allowance verification checks</h2>
    <table>
      <thead><tr><th>Role</th><th>Symbol</th><th>Token</th><th>Expected allowance</th><th>Allowance exact</th><th>Balance covers</th></tr></thead>
      <tbody>${allowanceRows || '<tr><td colspan="6">No allowance checks.</td></tr>'}</tbody>
    </table>
  </section>

  <section class="card">
    <h2>Required before liquidity mint calldata generation approval</h2>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      Token approvals and allowances are verified. Liquidity calldata has not been generated, liquidity has not been added,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-token-approval-post-execution-allowances">/api/public/dex-liquidity-token-approval-post-execution-allowances</a></p>
    <p><a href="/dex-liquidity-token-approval-safe-execution-live">DEX Token Approval Safe Execution Live</a></p>
...
console.log("AstraTreasury DEX Token Approval Post-Execution Allowances");
console.log("===========================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Allowance verification complete: ${publicPayload.summary.tokenApprovalPostExecutionAllowanceVerificationComplete}`);
console.log(`All allowances available: ${publicPayload.summary.allRequiredAllowancesAvailable}`);
