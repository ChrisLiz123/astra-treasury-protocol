...
const reportDir = path.join(root, "reports", "restricted-mode-evidence-seal");
const reportFile = path.join(reportDir, "restricted-mode-evidence-seal.json");

const publicJsonFile = path.join(root, "public-docs", "restricted-mode-evidence-seal-status.json");
const publicHtmlFile = path.join(root, "public-docs", "restricted-mode-evidence-seal.html");
...
const config = readJson("configs/restricted-mode-evidence-seal.config.json");
...
for (const artifactPath of missingArtifacts) {
  check(checks, `Artifact exists: ${artifactPath}`, false, { artifactPath });
}

const artifactHashes = requiredArtifactPaths
  .filter((artifactPath) => fs.existsSync(path.join(root, artifactPath)))
  .map((artifactPath) => sha256File(artifactPath));

const sealPayload = {
  schema: "astra-restricted-mode-evidence-seal-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  sealScope: "post-governance-decision-restricted-mode-all-disabled",
  artifactHashes
};

const sealHash = sha256Json(sealPayload);
const failures = checks.filter((item) => !item.pass);

const status = failures.length === 0
  ? "RESTRICTED_MODE_EVIDENCE_SEALED_DECISION_RECORDED_ALL_DISABLED"
  : "RESTRICTED_MODE_EVIDENCE_SEAL_REVIEW_REQUIRED";

const report = {
  schema: "astra-restricted-mode-evidence-seal-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  sealScope: config.sealScope || "post-governance-decision-restricted-mode-all-disabled",
  publicStatement:
    "AstraTreasury restricted-mode evidence seal is generated for the recorded governance decision and all-disabled monitoring baseline. It does not approve any capability.",
  summary: {
...
const sealRows = artifactHashes.map((item) => {
...
<head>
  ...
  <title>AstraTreasury Restricted-Mode Evidence Seal</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Evidence sealed · restricted mode</div>
    <h1>Restricted-Mode Evidence Seal</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Seal hash:</strong> <code>${escapeHtml(sealHash)}</code></p>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This evidence seal is an integrity snapshot only. It does not approve launch, funding, public sale,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/restricted-mode-evidence-seal">/api/public/restricted-mode-evidence-seal</a></p>
...
console.log("AstraTreasury Restricted-Mode Evidence Seal");
console.log("===========================================");
console.log(`Status: ${report.status}`);
console.log(`Artifact hashes: ${report.summary.artifactCount}`);
