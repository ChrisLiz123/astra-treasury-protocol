...
const statusReportFile = path.join(reportDir, "dex-liquidity-safe-submission-preparation-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-safe-submission-preparation-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-safe-submission-preparation.html");
...
const prepared = Boolean(preparation && preparation.schema === "astra-dex-liquidity-safe-submission-preparation-v0.1");
...
const status = prepared
  ? preparation.status
  : "DEX_LIQUIDITY_SAFE_SUBMISSION_PREPARATION_NOT_RUN";
...
const txRows = (preparation?.preparedTransactions || []).map((item) => {
  return `<tr>
...
const publicPayload = {
  schema: "astra-dex-liquidity-safe-submission-preparation-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: prepared
    ? "AstraTreasury prepared DEX liquidity Safe submission instructions. The Safe transaction has not been submitted or executed, liquidity has not been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity Safe submission preparation has not completed successfully.",
  summary: {
    prepared,
    liquiditySafeSubmissionPreparationComplete: preparation?.liquiditySafeSubmissionPreparationComplete === true,
    operatorSubmissionCommandReviewed: preparation?.operatorSubmissionCommandReviewed === true,
    ...
    transactionCount: preparation?.transactionCount || 0,
    liquiditySafeSubmissionApproved: preparation?.liquiditySafeSubmissionApproved === true,
    liquiditySafePayloadGenerated: preparation?.liquiditySafePayloadGenerated === true,
    liquiditySafePayloadVerified: preparation?.liquiditySafePayloadVerified === true,
    ...
    nextRecommendedMilestone: "DEX Liquidity Safe Submission Dry Run",
    artifactCount: artifactHashes.length
  },
  liquiditySafeSubmissionPreparation: {
    preparationHash: preparation?.preparationHash || "",
    hashAlgorithm: "SHA-256",
    safePayloadReference: preparation?.safePayloadReference || "",
    transactionBuilderReference: preparation?.transactionBuilderReference || "",
    transactionBuilderImportInstruction: preparation?.transactionBuilderImportInstruction || "",
    safeUiInstruction: preparation?.safeUiInstruction || "",
    operatorWarnings: preparation?.operatorWarnings || [],
    preparedTransactions: preparation?.preparedTransactions || [],
    requiredBeforeLiquiditySafeSubmissionDryRun: preparation?.requiredBeforeLiquiditySafeSubmissionDryRun || {},
    artifactHashes
  },
  currentStatuses: {
...
const requiredRows = Object.entries(publicPayload.liquiditySafeSubmissionPreparation.requiredBeforeLiquiditySafeSubmissionDryRun || {}).map(([key, value]) => {
  return `<tr><td>${escapeHtml(key)}</td><td>${value ? "Complete" : "Required / pending"}</td></tr>`;
}).join("");

const warningRows = (publicPayload.liquiditySafeSubmissionPreparation.operatorWarnings || []).map((item) => {
...
<head>
  ...
  <title>AstraTreasury DEX Liquidity Safe Submission Preparation</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Submission prepared · not submitted · no liquidity</div>
    <h1>DEX Liquidity Safe Submission Preparation</h1>
    <p>${escapeHtml(publicPayload.publicStatement)}</p>
    <p><strong>Preparation hash:</strong> <code>${escapeHtml(publicPayload.liquiditySafeSubmissionPreparation.preparationHash)}</code></p>
  </section>
  ...
  <section class="card">
    ...
    <table>
      ...
      <tbody>${txRows || '<tr><td colspan="5">No prepared transactions.</td></tr>'}</tbody>
    </table>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This is preparation only. The Safe transaction has not been submitted or executed,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-safe-submission-preparation">/api/public/dex-liquidity-safe-submission-preparation</a></p>
    <p><a href="/dex-liquidity-safe-submission-approval">DEX Liquidity Safe Submission Approval</a></p>
    <p><a href="/dex-liquidity-safe-payload-verification">DEX Liquidity Safe Payload Verification</a></p>
...
console.log("AstraTreasury DEX Liquidity Safe Submission Preparation");
console.log("=======================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Preparation complete: ${publicPayload.summary.liquiditySafeSubmissionPreparationComplete}`);
console.log(`Safe queue URL: ${publicPayload.summary.safeQueueUrl}`);
