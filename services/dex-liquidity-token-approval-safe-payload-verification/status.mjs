...
const reportDir = path.join(root, "reports", "dex-liquidity-token-approval-safe-payload-verification");
const statusReportFile = path.join(reportDir, "dex-liquidity-token-approval-safe-payload-verification-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-token-approval-safe-payload-verification-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-token-approval-safe-payload-verification.html");
...
const verificationPresent = Boolean(verification && verification.schema === "astra-dex-liquidity-token-approval-safe-payload-verification-v0.1");
...
const status = verificationPresent
  ? verification.status
  : "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_PAYLOAD_VERIFICATION_NOT_RUN";
...
const checkRows = (verification?.transactionChecks || []).map((check) => {
  return `<tr>
    ...
    <td>${escapeHtml(check.approvalSpenderMatches)}</td>
    <td>${escapeHtml(check.amountMatchesCalldata)}</td>
    <td>${escapeHtml(check.transactionBuilderMatchesPayload)}</td>
  </tr>`;
}).join("");

const publicPayload = {
  schema: "astra-dex-liquidity-token-approval-safe-payload-verification-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: status === "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_PAYLOAD_VERIFIED_NOT_SUBMITTED_NO_APPROVALS_EXECUTED_NO_LIQUIDITY"
    ? "AstraTreasury verified the generated DEX liquidity token-approval Safe payload. The Safe transaction has not been submitted, no token approval has executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity token-approval Safe payload verification has not completed successfully.",
  summary: {
    verificationPresent,
    tokenApprovalSafePayloadVerified: status === "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_PAYLOAD_VERIFIED_NOT_SUBMITTED_NO_APPROVALS_EXECUTED_NO_LIQUIDITY",
    tokenApprovalPayloadHashVerified: verification?.tokenApprovalPayloadHashVerified === true,
    transactionBuilderHashVerified: verification?.transactionBuilderHashVerified === true,
    approvalCalldataVerified: verification?.approvalCalldataVerified === true,
    ...
    nextRecommendedMilestone: "DEX Liquidity Token Approval Safe Submission Approval",
    artifactCount: artifactHashes.length
  },
  tokenApprovalSafePayloadVerification: {
    verificationHash: verification?.verificationHash || "",
    hashAlgorithm: "SHA-256",
    transactionChecks: verification?.transactionChecks || [],
    requiredBeforeTokenApprovalSafeSubmissionApproval: verification?.requiredBeforeTokenApprovalSafeSubmissionApproval || {},
    artifactHashes
  },
  currentStatuses: {
...
const requiredRows = Object.entries(publicPayload.tokenApprovalSafePayloadVerification.requiredBeforeTokenApprovalSafeSubmissionApproval || {}).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Token Approval Safe Payload Verification</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Token approval payload verified · not submitted · no approvals executed</div>
    <h1>DEX Token Approval Safe Payload Verification</h1>
    <p>${escapeHtml(publicPayload.publicStatement)}</p>
    <p><strong>Verification hash:</strong> <code>${escapeHtml(publicPayload.tokenApprovalSafePayloadVerification.verificationHash)}</code></p>
  </section>
  ...
  <section class="card">
    ...
    <table>
      <thead><tr><th>Role</th><th>Symbol</th><th>Token</th><th>Mode</th><th>Spender matches</th><th>Amount matches</th><th>Builder matches</th></tr></thead>
      <tbody>${checkRows || '<tr><td colspan="7">No transaction checks.</td></tr>'}</tbody>
    </table>
  </section>

  <section class="card">
    <h2>Required before Safe submission approval</h2>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This is verification only. The Safe transaction has not been submitted, token approvals have not executed,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-token-approval-safe-payload-verification">/api/public/dex-liquidity-token-approval-safe-payload-verification</a></p>
...
console.log("AstraTreasury DEX Token Approval Safe Payload Verification");
console.log("==========================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Payload verified: ${publicPayload.summary.tokenApprovalSafePayloadVerified}`);
console.log(`Payload hash verified: ${publicPayload.summary.tokenApprovalPayloadHashVerified}`);
