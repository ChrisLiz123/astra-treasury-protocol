...
const reportDir = path.join(root, "reports", "capability-request-review");
const reportFile = path.join(reportDir, "capability-request-review-status.json");

const publicJsonFile = path.join(root, "public-docs", "capability-request-review-status.json");
const publicHtmlFile = path.join(root, "public-docs", "capability-request-review.html");
...
check(checks, "Request review gate prepared", config.requestReviewGatePrepared === true, {
  requestReviewGatePrepared: config.requestReviewGatePrepared
});

check(checks, "Request import status healthy", [
...
check(checks, "Capability intake status healthy", [
  ...
  if (request) {
    ...
    for (const key of [
      ...
      if (request[key] !== false) {
        requestIssues.push(`${key} must be false for review.`);
      }
    }
...
const missingArtifacts = reviewArtifactPaths.filter((artifactPath) => {
  return !fs.existsSync(path.join(root, artifactPath));
});

for (const artifactPath of missingArtifacts) {
  check(checks, `Review artifact exists: ${artifactPath}`, false, { artifactPath });
}

const artifactHashes = reviewArtifactPaths
...
const reviewPayload = {
  schema: "astra-capability-request-review-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  requestReviewScope: "review-imported-capability-request-only",
  requestFilePresent: fs.existsSync(requestPath),
  requestValid,
  artifactHashes
};

const requestReviewHash = sha256Json(reviewPayload);
const failures = checks.filter((item) => !item.pass);

const status = failures.length > 0
  ? "CAPABILITY_REQUEST_REVIEW_GATE_REVIEW_REQUIRED"
  : requestValid
    ? "CAPABILITY_REQUEST_REVIEW_READY_TO_OPEN_ACTION_PATH_NO_APPROVAL"
    : "CAPABILITY_REQUEST_REVIEW_GATE_READY_NO_ACTIVE_REQUEST";

const reviewOutcome = requestValid
  ? {
      outcome: "READY_TO_OPEN_DEDICATED_ACTION_PATH_NO_APPROVAL",
      requestedCapability: request.requestedCapability,
      nextRecommendedMilestone: `Dedicated ${request.requestedCapability} Action-Specific Review Path`,
      capabilityApproved: false,
      safePayloadGenerated: false,
      executionQueueEnabled: false,
      fundsMovementApproved: false
    }
  : {
      outcome: "NO_ACTIVE_REQUEST",
      requestedCapability: "none",
      nextRecommendedMilestone: "Import a real capability request if future activation review is desired.",
      capabilityApproved: false,
      safePayloadGenerated: false,
      executionQueueEnabled: false,
      fundsMovementApproved: false
...
const report = {
  schema: "astra-capability-request-review-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  requestReviewScope: config.requestReviewScope || "review-imported-capability-request-only",
  publicStatement:
    requestValid
      ? "AstraTreasury capability request review is ready to open a dedicated action-specific path. No capability is approved."
      : "AstraTreasury capability request review gate is ready. No active request is present, and all restricted capabilities remain disabled and not approved.",
  summary: {
    requestFilePresent: fs.existsSync(requestPath),
    requestValid,
...
<head>
  ...
  <title>AstraTreasury Capability Request Review</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Request review · no capability approved</div>
    <h1>Capability Request Review</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Request review hash:</strong> <code>${escapeHtml(requestReviewHash)}</code></p>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      Request review can recommend opening a dedicated action-specific path. It does not approve launch,
      funding, public sale, staking/rewards, buybacks, execution, automation, autonomous execution,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/capability-request-review">/api/public/capability-request-review</a></p>
    <p><a href="/capability-request-import">Capability Request Import</a></p>
...
console.log("AstraTreasury Capability Request Review Gate");
console.log("============================================");
...
console.log(`Request review hash: ${requestReviewHash}`);
