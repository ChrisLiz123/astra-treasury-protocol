...
const statusReportFile = path.join(reportDir, "dex-liquidity-funding-transfer-safe-pending-signatures-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-pending-signatures-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-pending-signatures.html");
...
const reportPresent = Boolean(report && report.schema === "astra-dex-liquidity-funding-transfer-safe-pending-signatures-monitoring-v0.1");
...
const status = reportPresent
  ? report.status
  : "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_PENDING_SIGNATURES_MONITORING_NOT_RUN";
...
const txRows = (report?.transactionChecks || []).map((check) => {
  return `<tr>
...
const publicPayload = {
  schema: "astra-dex-liquidity-funding-transfer-safe-pending-signatures-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: reportPresent
    ? thresholdReached
      ? "AstraTreasury monitored the submitted DEX liquidity funding-transfer Safe transaction. The required Safe threshold is reached, but the Safe transaction is not executed, no treasury funds have moved, no token approvals have executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
      : "AstraTreasury monitored the submitted DEX liquidity funding-transfer Safe transaction. Additional Safe confirmations are still pending, the Safe transaction is not executed, no treasury funds have moved, no token approvals have executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity funding-transfer Safe pending-signatures monitoring has not yet run.",
  summary: {
    reportPresent,
    ...
    thresholdReached,
    safeTransactionSubmitted: report?.safeTransactionSubmitted === true,
    safeTransactionExecuted: false,
    fundingTransferSubmitted: report?.fundingTransferSubmitted === true,
    ...
    nextRecommendedMilestone: thresholdReached
      ? "DEX Liquidity Treasury Funding Transfer Safe Execution Approval"
      : "DEX Liquidity Treasury Funding Transfer Safe Pending Signatures Monitoring",
  ...
  pendingSignaturesMonitoring: {
    ...
    transactionChecks: report?.transactionChecks || [],
    requiredBeforeFundingTransferExecutionApproval: report?.requiredBeforeFundingTransferExecutionApproval || {},
    artifactHashes
  },
  currentStatuses: {
...
const requiredRows = Object.entries(publicPayload.pendingSignaturesMonitoring.requiredBeforeFundingTransferExecutionApproval || {}).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Funding Transfer Safe Pending Signatures</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Pending signatures monitoring · not executed · no funds moved</div>
    <h1>DEX Funding Transfer Safe Pending Signatures</h1>
  ...
  <section class="card">
    <h2>Transfer readiness checks</h2>
    <table>
      ...
      <tbody>${txRows || '<tr><td colspan="6">No transaction checks.</td></tr>'}</tbody>
    </table>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This is monitoring only. It has not executed the Safe transaction, and no funds have moved.
      Execution requires a separate future approval.
    </div>
  </section>

  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-funding-transfer-safe-pending-signatures">/api/public/dex-liquidity-funding-transfer-safe-pending-signatures</a></p>
...
console.log("AstraTreasury DEX Funding Transfer Safe Pending Signatures");
console.log("==========================================================");
