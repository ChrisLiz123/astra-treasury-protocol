...
const server = http.createServer((req, res) => {
  try {
    ...
    if (url.pathname === "/dex-liquidity-funding-transfer-safe-submission-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-submission-preparation.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity funding transfer Safe submission preparation page not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-submission-preparation:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-funding-transfer-safe-submission-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-submission-preparation-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity funding transfer Safe submission preparation status not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-submission-preparation:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-funding-transfer-safe-submission-dry-run") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-submission-dry-run.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity funding transfer Safe submission dry-run page not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-submission-dry-run:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-funding-transfer-safe-submission-dry-run") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-submission-dry-run-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity funding transfer Safe submission dry-run status not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-submission-dry-run:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-funding-transfer-safe-submission-live") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-submission-live.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity funding transfer Safe submission live page not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-submission-live:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-funding-transfer-safe-submission-live") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-submission-live-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity funding transfer Safe submission live status not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-submission-live:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-funding-transfer-safe-pending-signatures") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-pending-signatures.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity funding transfer Safe pending signatures page not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-pending-signatures:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-funding-transfer-safe-pending-signatures") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-pending-signatures-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity funding transfer Safe pending signatures status not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-pending-signatures:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-funding-transfer-safe-execution-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-execution-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity funding transfer Safe execution approval page not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-execution-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-funding-transfer-safe-execution-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-execution-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity funding transfer Safe execution approval status not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-execution-approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-funding-transfer-safe-execution-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-execution-preparation.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity funding transfer Safe execution preparation page not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-execution-preparation:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-funding-transfer-safe-execution-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-safe-execution-preparation-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity funding transfer Safe execution preparation status not generated yet. Run: npm run dex:liquidity-funding-transfer-safe-execution-preparation:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-funding-transfer-post-execution-balances") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-post-execution-balances.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity funding transfer post-execution balances page not generated yet. Run: npm run dex:liquidity-funding-transfer-post-execution-balances:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-funding-transfer-post-execution-balances") {
      const filePath = path.join(docsDir, "dex-liquidity-funding-transfer-post-execution-balances-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity funding transfer post-execution balances status not generated yet. Run: npm run dex:liquidity-funding-transfer-post-execution-balances:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-requirements-recheck") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-requirements-recheck.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval requirements recheck page not generated yet. Run: npm run dex:liquidity-token-approval-recheck:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-requirements-recheck") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-requirements-recheck-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval requirements recheck status not generated yet. Run: npm run dex:liquidity-token-approval-recheck:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-payload-generation-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-payload-generation-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval payload generation approval page not generated yet. Run: npm run dex:liquidity-token-approval-payload-generation-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-payload-generation-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-payload-generation-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval payload generation approval status not generated yet. Run: npm run dex:liquidity-token-approval-payload-generation-approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-payload") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-payload.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe payload page not generated yet. Run: npm run dex:liquidity-token-approval-payload:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-payload") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-payload-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe payload status not generated yet. Run: npm run dex:liquidity-token-approval-payload:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-submission-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe submission approval page not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-submission-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe submission approval status not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-submission-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-preparation.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe submission preparation page not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-preparation:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-submission-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-preparation-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe submission preparation status not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-preparation:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-submission-dry-run") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-dry-run.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe submission dry-run page not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-dry-run:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-submission-dry-run") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-dry-run-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe submission dry-run status not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-dry-run:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-submission-live") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-live.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe submission live page not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-live:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-submission-live") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-submission-live-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe submission live status not generated yet. Run: npm run dex:liquidity-token-approval-safe-submission-live:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-pending-signatures") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-pending-signatures.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe pending signatures page not generated yet. Run: npm run dex:liquidity-token-approval-safe-pending-signatures:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-pending-signatures") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-pending-signatures-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe pending signatures status not generated yet. Run: npm run dex:liquidity-token-approval-safe-pending-signatures:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-execution-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-execution-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe execution approval page not generated yet. Run: npm run dex:liquidity-token-approval-safe-execution-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-execution-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-execution-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe execution approval status not generated yet. Run: npm run dex:liquidity-token-approval-safe-execution-approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-safe-execution-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-execution-preparation.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval Safe execution preparation page not generated yet. Run: npm run dex:liquidity-token-approval-safe-execution-preparation:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-safe-execution-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-safe-execution-preparation-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval Safe execution preparation status not generated yet. Run: npm run dex:liquidity-token-approval-safe-execution-preparation:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-token-approval-post-execution-allowances") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-post-execution-allowances.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity token approval post-execution allowances page not generated yet. Run: npm run dex:liquidity-token-approval-post-execution-allowances:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-token-approval-post-execution-allowances") {
      const filePath = path.join(docsDir, "dex-liquidity-token-approval-post-execution-allowances-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity token approval post-execution allowances status not generated yet. Run: npm run dex:liquidity-token-approval-post-execution-allowances:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-mint-calldata-verification") {
      const filePath = path.join(docsDir, "dex-liquidity-mint-calldata-verification.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity mint calldata verification page not generated yet. Run: npm run dex:liquidity-mint-calldata-verification:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-mint-calldata-verification") {
      const filePath = path.join(docsDir, "dex-liquidity-mint-calldata-verification-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity mint calldata verification status not generated yet. Run: npm run dex:liquidity-mint-calldata-verification:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-payload-generation-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-payload-generation-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe payload generation approval page not generated yet. Run: npm run dex:liquidity-safe-payload-generation-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-payload-generation-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-payload-generation-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe payload generation approval status not generated yet. Run: npm run dex:liquidity-safe-payload-generation-approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-payload") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-payload.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe payload page not generated yet. Run: npm run dex:liquidity-safe-payload:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-payload") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-payload-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe payload status not generated yet. Run: npm run dex:liquidity-safe-payload:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-payload-verification") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-payload-verification.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe payload verification page not generated yet. Run: npm run dex:liquidity-safe-payload-verification:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-payload-verification") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-payload-verification-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe payload verification status not generated yet. Run: npm run dex:liquidity-safe-payload-verification:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-submission-dry-run") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-submission-dry-run.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe submission dry-run page not generated yet. Run: npm run dex:liquidity-safe-submission-dry-run:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-submission-dry-run") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-submission-dry-run-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe submission dry-run status not generated yet. Run: npm run dex:liquidity-safe-submission-dry-run:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-pending-signatures") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-pending-signatures.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe pending signatures page not generated yet. Run: npm run dex:liquidity-safe-pending-signatures:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-pending-signatures") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-pending-signatures-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe pending signatures status not generated yet. Run: npm run dex:liquidity-safe-pending-signatures:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-execution-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-execution-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe execution approval page not generated yet. Run: npm run dex:liquidity-safe-execution-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-execution-approval") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-execution-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe execution approval status not generated yet. Run: npm run dex:liquidity-safe-execution-approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-execution-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-execution-preparation.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe execution preparation page not generated yet. Run: npm run dex:liquidity-safe-execution-preparation:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-execution-preparation") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-execution-preparation-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe execution preparation status not generated yet. Run: npm run dex:liquidity-safe-execution-preparation:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-safe-execution-live") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-execution-live.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity Safe execution live page not generated yet. Run: npm run dex:liquidity-safe-execution-live:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-safe-execution-live") {
      const filePath = path.join(docsDir, "dex-liquidity-safe-execution-live-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity Safe execution live status not generated yet. Run: npm run dex:liquidity-safe-execution-live:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-liquidity-post-execution-verification") {
      const filePath = path.join(docsDir, "dex-liquidity-post-execution-verification.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX liquidity post-execution verification page not generated yet. Run: npm run dex:liquidity-post-execution-verification:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-liquidity-post-execution-verification") {
      const filePath = path.join(docsDir, "dex-liquidity-post-execution-verification-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX liquidity post-execution verification status not generated yet. Run: npm run dex:liquidity-post-execution-verification:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/dex-buy-page-activation-approval") {
      const filePath = path.join(docsDir, "dex-buy-page-activation-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX buy page activation approval page not generated yet. Run: npm run dex:buy-page-activation-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-buy-page-activation-approval") {
      const filePath = path.join(docsDir, "dex-buy-page-activation-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX buy page activation approval status not generated yet. Run: npm run dex:buy-page-activation-approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/buy") {
      const filePath = path.join(docsDir, "buy.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX buy page is not activated yet.\n", 404);
        return;
      }
    ...
    if (url.pathname === "/dex-buy-page-activation-live") {
      const filePath = path.join(docsDir, "dex-buy-page-activation-live.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "DEX buy page activation live page not generated yet. Run: npm run dex:buy-page-activation-live:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/dex-buy-page-activation-live") {
      const filePath = path.join(docsDir, "dex-buy-page-activation-live-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "DEX buy page activation live status not generated yet. Run: npm run dex:buy-page-activation-live:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/full-launch-approval") {
      const filePath = path.join(docsDir, "full-launch-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "Full launch approval page not generated yet. Run: npm run full-launch:approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/full-launch-approval") {
      const filePath = path.join(docsDir, "full-launch-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "Full launch approval status not generated yet. Run: npm run full-launch:approval:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/global-treasury-funding-approval-review") {
      const filePath = path.join(docsDir, "global-treasury-funding-approval-review.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "Global treasury funding approval review page not generated yet. Run: npm run treasury:global-funding-approval-review:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/global-treasury-funding-approval-review") {
      const filePath = path.join(docsDir, "global-treasury-funding-approval-review-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "Global treasury funding approval review status not generated yet. Run: npm run treasury:global-funding-approval-review:status" }, 404);
        return;
      }
    ...
    if (url.pathname === "/global-treasury-funding-approval") {
      const filePath = path.join(docsDir, "global-treasury-funding-approval.html");

      if (!fs.existsSync(filePath)) {
        textResponse(res, "Global treasury funding approval page not generated yet. Run: npm run treasury:global-funding-approval:status\n", 404);
        return;
      }
    ...
    if (url.pathname === "/api/public/global-treasury-funding-approval") {
      const filePath = path.join(docsDir, "global-treasury-funding-approval-status.json");

      if (!fs.existsSync(filePath)) {
        jsonResponse(res, { error: "Global treasury funding approval status not generated yet. Run: npm run treasury:global-funding-approval:status" }, 404);
        return;
      }
