...
const reportDir = path.join(root, "reports", "dex-pool-creation-safe-submission-execution-approval");
const reportFile = path.join(reportDir, "dex-pool-creation-safe-submission-execution-approval-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-pool-creation-safe-submission-execution-approval-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-pool-creation-safe-submission-execution-approval.html");
...
const approvalRecordPresent = Boolean(record && record.schema === "astra-dex-pool-creation-safe-submission-execution-approval-record-v0.1");
...
check("Safe submission dry run ready", dryRun.status === "DEX_POOL_CREATION_SAFE_SUBMISSION_DRY_RUN_REVIEW_READY_NOT_SUBMITTED", {
  status: dryRun.status || "UNKNOWN"
});

check("Safe submission preparation ready", preparation.status === "DEX_POOL_CREATION_SAFE_SUBMISSION_PREPARATION_READY_NOT_SUBMITTED", {
  status: preparation.status || "UNKNOWN"
});

check("Safe submission approval recorded", submissionApproval.status === "DEX_POOL_CREATION_SAFE_SUBMISSION_APPROVED_NOT_SUBMITTED", {
  status: submissionApproval.status || "UNKNOWN"
});
...
check("Payload generated not executed", generation.status === "DEX_POOL_CREATION_SAFE_PAYLOAD_GENERATED_NOT_EXECUTED", {
...
check("No submission or execution in dry run", dryRun.summary?.safeTransactionSubmitted === false && dryRun.summary?.safeTransactionQueued === false && dryRun.summary?.safeTransactionExecuted === false, {
  safeTransactionSubmitted: dryRun.summary?.safeTransactionSubmitted,
  safeTransactionQueued: dryRun.summary?.safeTransactionQueued,
  safeTransactionExecuted: dryRun.summary?.safeTransactionExecuted
});
...
check("Approval record matches config", config.safeSubmissionExecutionApprovalRecorded === approvalRecordPresent && config.safeSubmissionExecutionApproved === approvalRecordPresent, {
  approvalRecordPresent,
  configSafeSubmissionExecutionApprovalRecorded: config.safeSubmissionExecutionApprovalRecorded,
  configSafeSubmissionExecutionApproved: config.safeSubmissionExecutionApproved
});
...
for (const artifactPath of missingArtifacts) {
  check(`Submission execution approval artifact exists: ${artifactPath}`, false, { artifactPath });
}
...
const approvalPayload = {
  schema: "astra-dex-pool-creation-safe-submission-execution-approval-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  selectedPublicPurchasePath: "dex-liquidity-pool-trading",
  approvalRecordPresent,
  safeSubmissionExecutionApproved: approvalRecordPresent,
...
const status = failures.length > 0
  ? "DEX_POOL_CREATION_SAFE_SUBMISSION_EXECUTION_APPROVAL_REVIEW_REQUIRED"
  : approvalRecordPresent
    ? "DEX_POOL_CREATION_SAFE_SUBMISSION_EXECUTION_APPROVED_NOT_SUBMITTED"
    : "DEX_POOL_CREATION_SAFE_SUBMISSION_EXECUTION_APPROVAL_READY_PENDING_APPROVAL";
...
const approvalSummary = approvalRecordPresent
  ...
  : {
      ...
      safeSubmissionExecutionApproved: false,
      safeTransactionSubmitted: false,
      safeTransactionQueued: false,
      safeTransactionExecuted: false,
      poolCreated: false,
...
const report = {
  schema: "astra-dex-pool-creation-safe-submission-execution-approval-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: approvalRecordPresent
    ? "AstraTreasury DEX pool creation Safe submission action is approved for the later dedicated submission step. The transaction has not been submitted to Safe, not queued, not executed, no pool is created, no liquidity is added, no funds are moved, and public trading is not approved."
    : "AstraTreasury DEX pool creation Safe submission execution approval framework is ready. Approval has not yet been recorded.",
  summary: {
    approvalRecordPresent,
    safeSubmissionExecutionApproved: approvalRecordPresent,
    safeTransactionSubmitted: false,
    ...
    fundsMoved: false,
    publicTradingApproved: false,
    nextRecommendedMilestone: "DEX Pool Creation Safe Submission Live",
  ...
  safeSubmissionExecutionApproval: {
    approvalHash,
...
const requiredRows = Object.entries(report.safeSubmissionExecutionApproval.requiredBeforeActualSafeSubmission).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Safe Submission Execution Approval</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Safe submission execution approval · not submitted</div>
    <h1>DEX Safe Submission Execution Approval</h1>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This page records approval for the later Safe submission action only. It does not call Safe APIs,
      open Safe UI, submit to Safe, request signatures, queue, execute, create a pool, add liquidity,
      move funds, or approve public trading.
    </div>
  </section>

  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-pool-creation-safe-submission-execution-approval">/api/public/dex-pool-creation-safe-submission-execution-approval</a></p>
    <p><a href="/dex-pool-creation-safe-submission-dry-run">DEX Safe Submission Dry Run</a></p>
    <p><a href="/dex-pool-creation-safe-submission-preparation">DEX Safe Submission Preparation</a></p>
...
console.log("AstraTreasury DEX Safe Submission Execution Approval");
console.log("====================================================");
console.log(`Status: ${report.status}`);
console.log(`Approval record present: ${approvalRecordPresent}`);
console.log(`Safe submission execution approved: ${report.summary.safeSubmissionExecutionApproved}`);
