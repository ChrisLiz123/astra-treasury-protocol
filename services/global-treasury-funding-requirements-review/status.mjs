...
const statusReportFile = path.join(reportDir, "global-treasury-funding-requirements-review-status.json");

const publicJsonFile = path.join(root, "public-docs", "global-treasury-funding-requirements-review-status.json");
const publicHtmlFile = path.join(root, "public-docs", "global-treasury-funding-requirements-review.html");
...
const reviewed = Boolean(review && review.schema === "astra-global-treasury-funding-requirements-review-v0.1");
...
const status = reviewed
  ? review.status
  : "GLOBAL_TREASURY_FUNDING_REQUIREMENTS_REVIEW_NOT_RUN";
...
const publicPayload = {
  schema: "astra-global-treasury-funding-requirements-review-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "public-launch-finalized-global-treasury-funding-approved",
  publicStatement: reviewed
    ? "AstraTreasury completed global treasury funding requirements review. Global treasury funding is approved, but no funding payload has been generated, no Safe transaction has been submitted, and no funds have been moved."
    : "AstraTreasury global treasury funding requirements review has not completed successfully.",
  summary: {
    reviewed,
    globalTreasuryFundingRequirementsReviewComplete: review?.globalTreasuryFundingRequirementsReviewComplete === true,
    readyForGlobalTreasuryFundingPayloadGenerationApproval: review?.readyForGlobalTreasuryFundingPayloadGenerationApproval === true,
    globalTreasuryFundingApprovalRecorded: review?.globalTreasuryFundingApprovalRecorded === true,
    globalTreasuryFundingApproved: review?.globalTreasuryFundingApproved === true,
    treasuryFundingApproved: review?.treasuryFundingApproved === true,
    ...
    nextRecommendedMilestone: "Global Treasury Funding Payload Generation Approval",
    artifactCount: artifactHashes.length
  },
  globalTreasuryFundingRequirementsReview: {
    reviewHash: review?.reviewHash || "",
    hashAlgorithm: "SHA-256",
    fundingRequirements: review?.fundingRequirements || {},
    requirementsChecks: review?.requirementsChecks || {},
    readinessNotes: review?.readinessNotes || [],
    requiredBeforeGlobalTreasuryFundingPayloadGenerationApproval: review?.requiredBeforeGlobalTreasuryFundingPayloadGenerationApproval || {},
    artifactHashes
  },
  currentStatuses: {
    globalTreasuryFundingApproval: approvalStatus.status || "UNKNOWN",
  ...
  restrictions: {
    fundingPayloadGeneration: false,
    safeSubmission: false,
...
const requirementRows = Object.entries(publicPayload.globalTreasuryFundingRequirementsReview.fundingRequirements || {}).map(([key, value]) => {
  return `<tr><td>${escapeHtml(key)}</td><td>${escapeHtml(value)}</td></tr>`;
}).join("");

const checkRows = Object.entries(publicPayload.globalTreasuryFundingRequirementsReview.requirementsChecks || {}).map(([key, value]) => {
  return `<tr><td>${escapeHtml(key)}</td><td>${escapeHtml(value)}</td></tr>`;
}).join("");

const requiredRows = Object.entries(publicPayload.globalTreasuryFundingRequirementsReview.requiredBeforeGlobalTreasuryFundingPayloadGenerationApproval || {}).map(([key, value]) => {
  return `<tr><td>${escapeHtml(key)}</td><td>${escapeHtml(value)}</td></tr>`;
}).join("");

const notesRows = (publicPayload.globalTreasuryFundingRequirementsReview.readinessNotes || []).map((item) => {
...
<head>
  ...
  <title>AstraTreasury Global Treasury Funding Requirements Review</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Requirements reviewed · no payload · no funds moved</div>
    <h1>Global Treasury Funding Requirements Review</h1>
    <p>${escapeHtml(publicPayload.publicStatement)}</p>
    <p><strong>Review hash:</strong> <code>${escapeHtml(publicPayload.globalTreasuryFundingRequirementsReview.reviewHash)}</code></p>
  </section>
  ...
  <section class="card">
    ...
    <table><thead><tr><th>Requirement</th><th>Value</th></tr></thead><tbody>${requirementRows}</tbody></table>
  </section>

  <section class="card">
    <h2>Requirements checks</h2>
    <table><thead><tr><th>Check</th><th>Status</th></tr></thead><tbody>${checkRows}</tbody></table>
  </section>

  <section class="card">
    <h2>Required before payload generation approval</h2>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This is requirements review only. Funding payload generation, Safe submission, execution, and fund movement require later milestones.
    </div>
  </section>

  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/global-treasury-funding-requirements-review">/api/public/global-treasury-funding-requirements-review</a></p>
    <p><a href="/global-treasury-funding-approval">Global Treasury Funding Approval</a></p>
...
console.log("AstraTreasury Global Treasury Funding Requirements Review");
console.log("=========================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Review complete: ${publicPayload.summary.globalTreasuryFundingRequirementsReviewComplete}`);
console.log(`Ready for payload generation approval: ${publicPayload.summary.readyForGlobalTreasuryFundingPayloadGenerationApproval}`);
console.log(`Funding payload generated: ${publicPayload.summary.fundingPayloadGenerated}`);
console.log(`Funds moved: ${publicPayload.summary.fundsMoved}`);
