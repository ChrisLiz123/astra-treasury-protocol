...
const reportDir = path.join(root, "reports", "dex-liquidity-funding-transfer-safe-submission-approval");
const statusReportFile = path.join(reportDir, "dex-liquidity-funding-transfer-safe-submission-approval-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-submission-approval-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-submission-approval.html");
...
const approvalRecordPresent = Boolean(record && record.schema === "astra-dex-liquidity-funding-transfer-safe-submission-approval-record-v0.1");
...
check("Payload verification complete", payloadVerificationStatus.status === "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_PAYLOAD_VERIFIED_NOT_SUBMITTED_NO_FUNDS_MOVED", {
  status: payloadVerificationStatus.status || "UNKNOWN"
});

check("Payload hash verified", payloadVerificationStatus.summary?.payloadHashVerified === true, {
  payloadHashVerified: payloadVerificationStatus.summary?.payloadHashVerified
});

check("No submission or funds movement", payloadVerificationStatus.summary?.fundingTransferSubmitted === false && payloadVerificationStatus.summary?.fundingTransferExecuted === false && payloadVerificationStatus.summary?.treasuryFundsMoved === false, {
  fundingTransferSubmitted: payloadVerificationStatus.summary?.fundingTransferSubmitted,
  fundingTransferExecuted: payloadVerificationStatus.summary?.fundingTransferExecuted,
  treasuryFundsMoved: payloadVerificationStatus.summary?.treasuryFundsMoved
});

check("Payload generated", payloadStatus.status === "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_PAYLOAD_GENERATED_NOT_SUBMITTED_NO_FUNDS_MOVED", {
...
check("No live/submission/liquidity/public-trading artifacts present", forbiddenArtifactsPresent.length === 0, {
...
const approvalPayload = {
  schema: "astra-dex-liquidity-funding-transfer-safe-submission-approval-public-payload-v0.1",
  approvalRecordPresent,
  fundingTransferSafeSubmissionApproved: approvalRecordPresent,
...
const status = failures.length > 0
  ? "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_SUBMISSION_APPROVAL_REVIEW_REQUIRED"
  : approvalRecordPresent
    ? "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_SUBMISSION_APPROVED_NOT_SUBMITTED_NO_FUNDS_MOVED"
    : "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_SUBMISSION_APPROVAL_READY_PENDING_APPROVAL";
...
const approvalSummary = approvalRecordPresent
  ...
  : {
      ...
      fundingTransferSafeSubmissionApproved: false,
      fundingTransferSubmitted: false,
      fundingTransferExecuted: false,
      treasuryFundsMoved: false,
      tokenApprovalExecuted: false,
...
const report = {
  schema: "astra-dex-liquidity-funding-transfer-safe-submission-approval-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: approvalRecordPresent
    ? "AstraTreasury approved submission/proposal of the verified DEX liquidity funding-transfer Safe payload. The Safe transaction has not been submitted, no Safe transaction has executed, no treasury funds have moved, no token approvals have executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity funding-transfer Safe submission approval framework is ready. Approval has not yet been recorded.",
  summary: {
    approvalRecordPresent,
    fundingTransferSafeSubmissionApproved: approvalRecordPresent,
    ...
    publicTradingApproved: false,
    fullLaunchApproved: false,
    nextRecommendedMilestone: "DEX Liquidity Treasury Funding Transfer Safe Submission Preparation",
  ...
  fundingTransferSafeSubmissionApproval: {
    approvalHash,
    hashAlgorithm: "SHA-256",
    approvalSummary,
    requiredBeforeFundingTransferSafeSubmissionLive: config.requiredBeforeFundingTransferSafeSubmissionLive || {},
  ...
  restrictions: {
    fundingTransferSubmission: !approvalRecordPresent,
...
const requiredRows = Object.entries(report.fundingTransferSafeSubmissionApproval.requiredBeforeFundingTransferSafeSubmissionLive).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Funding Transfer Safe Submission Approval</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Safe submission approval · not submitted · no funds moved</div>
    <h1>DEX Funding Transfer Safe Submission Approval</h1>
  ...
  <section class="card">
    <h2>Required before live Safe submission</h2>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This approval does not submit or execute the Safe transaction. No funds have moved.
      Safe submission and execution require separate future steps.
    </div>
  </section>

  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-funding-transfer-safe-submission-approval">/api/public/dex-liquidity-funding-transfer-safe-submission-approval</a></p>
    <p><a href="/dex-liquidity-funding-transfer-safe-payload-verification">DEX Funding Transfer Payload Verification</a></p>
    <p><a href="/dex-liquidity-funding-transfer-safe-payload">DEX Funding Transfer Safe Payload</a></p>
...
console.log("AstraTreasury DEX Funding Transfer Safe Submission Approval");
console.log("===========================================================");
console.log(`Status: ${report.status}`);
console.log(`Approval record present: ${approvalRecordPresent}`);
console.log(`Safe submission approved: ${report.summary.fundingTransferSafeSubmissionApproved}`);
console.log(`Funding transfer submitted: ${report.summary.fundingTransferSubmitted}`);
