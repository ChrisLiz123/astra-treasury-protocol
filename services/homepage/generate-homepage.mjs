import fs from "node:fs";
import path from "node:path";
import { execSync } from "node:child_process";

const root = process.cwd();
const outHtml = path.join(root, "public-docs", "index.html");
const outJson = path.join(root, "public-docs", "home-status.json");

fs.mkdirSync(path.dirname(outHtml), { recursive: true });

function readJson(relativePath, fallback = {}) {
  try {
    const full = path.join(root, relativePath);
    if (!fs.existsSync(full)) return fallback;
    return JSON.parse(fs.readFileSync(full, "utf8"));
  } catch (error) {
    return { error: error.message };
  }
}

function run(command, fallback = "") {
  try {
    return execSync(command, {
      cwd: root,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"]
    }).trim();
  } catch {
    return fallback;
  }
}

function escapeHtml(value) {
  return String(value ?? "").replace(/[&<>"]/g, (ch) => ({
    "&": "&amp;",
    "<": "&lt;",
    ">": "&gt;",
    "\"": "&quot;"
  }[ch]));
}

function humanizeStatus(value, fallback = "Unknown") {
  if (!value) return fallback;

  const map = {
    RESTRICTED_LAUNCH_READY: "Restricted launch ready",
    RESTRICTED_LAUNCH_STABILIZED: "Restricted launch stabilized",
    RESTRICTED_LAUNCH_STABILIZATION_REVIEW_REQUIRED: "Stabilization under review",
    MAINNET_CONTRACTS_DEPLOYED_RESTRICTED_OPERATION: "Restricted mainnet operation",
    MAINNET_RESTRICTED_OPERATION: "Restricted operation",
    MAINNET_EXECUTION_QUEUE_DISABLED: "Execution queue disabled",
    PASS: "Healthy",
    FAIL: "Needs review",
    CLEAR: "Clear"
  };

  if (map[value]) return map[value];

  return String(value)
    .toLowerCase()
    .replace(/_/g, " ")
    .replace(/\b\w/g, (c) => c.toUpperCase());
}

function statusTone(value) {
  const v = String(value || "").toUpperCase();

  if (v.includes("FAIL") || v.includes("ERROR") || v.includes("CRITICAL")) return "danger";
  if (v.includes("REVIEW") || v.includes("WARN") || v.includes("PENDING")) return "warn";
  return "ok";
}

const mainnet = readJson("public-docs/mainnet-status.json");
const live = readJson("public-docs/live-status.json");
const launch = readJson("public-docs/restricted-launch-status.json");
const monitor = readJson("public-docs/mainnet-monitor-status.json");
const alerts = readJson("public-docs/mainnet-alerts-status.json");
const incidents = readJson("public-docs/incident-summary.json");
const roadmap = readJson("public-docs/capability-roadmap-status.json");
const stabilization = readJson("public-docs/stabilization-status.json");
const packages = readJson("public-docs/package-inventory.json");

const shortCommit = run("git rev-parse --short HEAD", "unknown");
const branch = run("git rev-parse --abbrev-ref HEAD", "unknown");

const contracts = mainnet.contracts || {};
const contractCount = Object.keys(contracts).length;

const activeIncidents = Number(incidents?.summary?.active || 0);
const responseRequired = Boolean(alerts?.responseRequired);

const rawLaunchStatus = launch?.status || "UNKNOWN";
const rawLiveStatus = live?.liveStatus || live?.status || "UNKNOWN";
const rawMonitorStatus = monitor?.status || "UNKNOWN";
const rawStabilizationStatus = stabilization?.status || "PENDING_24H_WINDOW";
const roadmapStatus = roadmap?.status || "UNKNOWN";

const alertDisplay = activeIncidents > 0 || responseRequired ? "Review required" : "Clear";
const alertTone = activeIncidents > 0 || responseRequired ? "warn" : "ok";

const status = {
  schema: "astra-home-status-v0.3",
  generatedAt: new Date().toISOString(),
  project: "AstraTreasury Protocol",
  version: "0.1.1",
  branch,
  shortCommit,
  publicStatement:
    "AstraTreasury is deployed on Base Mainnet in restricted operational mode. Public token sale, real treasury funding, staking and rewards, buybacks, autonomous execution, and mainnet execution queue activation remain disabled.",
  network: {
    name: "Base Mainnet",
    chainId: 8453
  },
  summary: {
    contractCount,
    launchStatus: rawLaunchStatus,
    liveStatus: rawLiveStatus,
    monitorStatus: rawMonitorStatus,
    stabilizationStatus: rawStabilizationStatus,
    roadmapStatus,
    alertsResponseRequired: responseRequired,
    activeIncidents,
    packageCount: packages?.summary?.total || null
  },
  restrictions: {
    publicTokenSale: false,
    realTreasuryFunding: false,
    stakingOrRewards: false,
    buybackProgram: false,
    autonomousExecution: false,
    mainnetExecutionQueue: false
  },
  primaryLinks: {
    mainnet: "/mainnet",
    live: "/live",
    monitor: "/monitor",
    launch: "/launch",
    announcement: "/announcement",
    transparency: "/transparency",
    roadmap: "/roadmap"
  }
};

fs.writeFileSync(outJson, JSON.stringify(status, null, 2) + "\n");

const contractRows = Object.entries(contracts).map(([name, address]) => `
  <tr>
    <td>${escapeHtml(name)}</td>
    <td><code>${escapeHtml(address)}</code></td>
    <td><a href="https://basescan.org/address/${escapeHtml(address)}" target="_blank" rel="noreferrer">BaseScan</a></td>
  </tr>
`).join("");

const html = `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>AstraTreasury Protocol</title>
  <meta name="description" content="AstraTreasury Protocol is a restricted Base Mainnet deployment for AI-assisted treasury governance workflows." />
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
      --surface: #0e1a2b;
      --surface-2: #111f33;
      --surface-3: #14243a;
      --border: rgba(148, 163, 184, 0.18);
      --border-strong: rgba(148, 163, 184, 0.28);
      --text: #edf4fb;
      --muted: #9aaec4;
      --muted-2: #71849a;
      --blue: #67a7ff;
      --green: #41d49b;
      --yellow: #f4c35f;
      --red: #f87171;
      --shadow: 0 22px 70px rgba(0, 0, 0, 0.28);
      font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
    }

    * {
      box-sizing: border-box;
    }

    body {
      margin: 0;
      min-height: 100vh;
      color: var(--text);
      background:
        radial-gradient(circle at 15% 0%, rgba(103, 167, 255, 0.16), transparent 30%),
        linear-gradient(180deg, #07101d 0%, #091426 52%, #08111f 100%);
    }

    a {
      color: inherit;
      text-decoration: none;
    }

    .page {
      width: min(1180px, calc(100% - 44px));
      margin: 0 auto;
    }

    .topbar {
      border-bottom: 1px solid var(--border);
      background: rgba(8, 17, 31, 0.78);
      backdrop-filter: blur(16px);
      position: sticky;
      top: 0;
      z-index: 5;
    }

    .nav {
      width: min(1180px, calc(100% - 44px));
      margin: 0 auto;
      min-height: 74px;
      display: flex;
      justify-content: space-between;
      align-items: center;
      gap: 20px;
    }

    .brand {
      display: flex;
      align-items: center;
      gap: 12px;
      font-weight: 800;
      letter-spacing: 0.1px;
      color: var(--text);
    }

    .mark {
      width: 38px;
      height: 38px;
      border-radius: 11px;
      background:
        linear-gradient(135deg, rgba(103, 167, 255, 0.95), rgba(65, 212, 155, 0.95));
      box-shadow: 0 0 0 1px rgba(255,255,255,.08), 0 18px 40px rgba(65,212,155,.16);
    }

    .nav-links {
      display: flex;
      align-items: center;
      gap: 6px;
      flex-wrap: wrap;
      justify-content: flex-end;
    }

    .nav-links a {
      color: var(--muted);
      font-size: 14px;
      font-weight: 650;
      padding: 9px 11px;
      border-radius: 10px;
    }

    .nav-links a:hover {
      color: var(--text);
      background: rgba(255,255,255,.045);
    }

    .hero {
      padding: 58px 0 28px;
      display: grid;
      grid-template-columns: 1.02fr 0.98fr;
      gap: 28px;
      align-items: center;
    }

    .eyebrow {
      display: inline-flex;
      align-items: center;
      gap: 8px;
      color: var(--green);
      background: rgba(65, 212, 155, 0.10);
      border: 1px solid rgba(65, 212, 155, 0.20);
      padding: 8px 12px;
      border-radius: 999px;
      font-size: 13px;
      font-weight: 750;
      margin-bottom: 20px;
    }

    .dot {
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background: var(--green);
      box-shadow: 0 0 18px rgba(65, 212, 155, .75);
    }

    h1 {
      margin: 0;
      font-size: clamp(38px, 5vw, 60px);
      line-height: 1.02;
      letter-spacing: -2.2px;
      max-width: 760px;
    }

    .hero-copy {
      margin: 20px 0 0;
      color: var(--muted);
      font-size: 18px;
      line-height: 1.68;
      max-width: 660px;
    }

    .cta-row {
      display: flex;
      gap: 12px;
      flex-wrap: wrap;
      margin-top: 28px;
    }

    .button {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      min-height: 46px;
      padding: 0 16px;
      border-radius: 11px;
      font-weight: 800;
      border: 1px solid var(--border-strong);
      background: rgba(255,255,255,.045);
      color: var(--text);
      transition: 150ms ease;
    }

    .button:hover {
      background: rgba(255,255,255,.07);
      transform: translateY(-1px);
    }

    .button.primary {
      background: var(--text);
      color: #07101d;
      border-color: transparent;
    }

    .button.primary:hover {
      background: #ffffff;
    }

    .hero-card {
      background: var(--surface);
      border: 1px solid var(--border);
      border-radius: 24px;
      box-shadow: var(--shadow);
      padding: 24px;
    }

    .snapshot-header {
      display: flex;
      justify-content: space-between;
      align-items: flex-start;
      gap: 16px;
      margin-bottom: 18px;
    }

    .snapshot-header h2 {
      margin: 0;
      font-size: 18px;
      letter-spacing: -0.3px;
    }

    .snapshot-header p {
      margin: 6px 0 0;
      color: var(--muted);
      font-size: 13px;
      line-height: 1.5;
    }

    .badge {
      white-space: nowrap;
      color: var(--green);
      background: rgba(65, 212, 155, .10);
      border: 1px solid rgba(65, 212, 155, .20);
      padding: 7px 10px;
      border-radius: 999px;
      font-size: 12px;
      font-weight: 800;
    }

    .status-table {
      display: grid;
      gap: 10px;
    }

    .status-row {
      display: grid;
      grid-template-columns: 155px 1fr;
      gap: 14px;
      align-items: center;
      border: 1px solid var(--border);
      background: rgba(255,255,255,.025);
      border-radius: 14px;
      padding: 14px;
    }

    .status-label {
      color: var(--muted);
      font-size: 13px;
    }

    .status-value {
      font-weight: 850;
      line-height: 1.35;
    }

    .ok { color: var(--green); }
    .warn { color: var(--yellow); }
    .danger { color: var(--red); }

    .disclosure {
      margin-top: 16px;
      padding: 14px;
      border-radius: 14px;
      border: 1px solid rgba(244,195,95,.24);
      background: rgba(244,195,95,.07);
      color: #f7d99a;
      font-size: 13px;
      line-height: 1.55;
    }

    .section {
      padding: 30px 0;
    }

    .section-head {
      display: flex;
      justify-content: space-between;
      align-items: flex-end;
      gap: 20px;
      margin-bottom: 16px;
    }

    .section-head h2 {
      margin: 0;
      font-size: 28px;
      letter-spacing: -0.8px;
    }

    .section-head p {
      margin: 0;
      max-width: 670px;
      color: var(--muted);
      line-height: 1.6;
    }

    .grid-3 {
      display: grid;
      grid-template-columns: repeat(3, 1fr);
      gap: 16px;
    }

    .panel {
      background: var(--surface);
      border: 1px solid var(--border);
      border-radius: 20px;
      padding: 22px;
      box-shadow: 0 18px 45px rgba(0,0,0,.18);
    }

    .panel h3 {
      margin: 0 0 9px;
      font-size: 18px;
      letter-spacing: -0.2px;
    }

    .panel p {
      margin: 0;
      color: var(--muted);
      line-height: 1.6;
    }

    .panel-kicker {
      color: var(--blue);
      font-size: 12px;
      font-weight: 850;
      letter-spacing: .08em;
      text-transform: uppercase;
      margin-bottom: 11px;
    }

    .contracts {
      background: var(--surface);
      border: 1px solid var(--border);
      border-radius: 20px;
      overflow: hidden;
      box-shadow: 0 18px 45px rgba(0,0,0,.18);
    }

    table {
      width: 100%;
      border-collapse: collapse;
    }

    th {
      text-align: left;
      color: var(--muted);
      font-size: 12px;
      text-transform: uppercase;
      letter-spacing: .08em;
      background: rgba(255,255,255,.03);
    }

    th, td {
      border-bottom: 1px solid var(--border);
      padding: 15px 18px;
      vertical-align: top;
    }

    tr:last-child td {
      border-bottom: 0;
    }

    td {
      color: var(--text);
      font-size: 14px;
    }

    td a {
      color: var(--blue);
      font-weight: 750;
    }

    code {
      color: var(--muted);
      font-size: 12px;
      overflow-wrap: anywhere;
    }

    .link-grid {
      display: grid;
      grid-template-columns: repeat(4, 1fr);
      gap: 12px;
    }

    .link-card {
      border: 1px solid var(--border);
      background: rgba(255,255,255,.028);
      border-radius: 16px;
      padding: 16px;
      transition: 150ms ease;
    }

    .link-card:hover {
      background: rgba(255,255,255,.055);
      transform: translateY(-1px);
    }

    .link-card strong {
      display: block;
      font-size: 15px;
      margin-bottom: 6px;
    }

    .link-card span {
      display: block;
      color: var(--muted);
      font-size: 13px;
      line-height: 1.5;
    }

    .footer {
      color: var(--muted-2);
      border-top: 1px solid var(--border);
      margin-top: 28px;
      padding: 26px 0 44px;
      font-size: 13px;
      line-height: 1.65;
    }

    @media (max-width: 940px) {
      .hero,
      .grid-3,
      .link-grid {
        grid-template-columns: 1fr;
      }

      .nav {
        align-items: flex-start;
        flex-direction: column;
      }

      .status-row {
        grid-template-columns: 1fr;
      }

      h1 {
        letter-spacing: -1.5px;
      }
    }
  </style>
</head>
<body>
  <div class="topbar">
    <nav class="nav" aria-label="Primary navigation">
      <a class="brand" href="/">
        <span class="mark" aria-hidden="true"></span>
        <span>AstraTreasury</span>
      </a>
      <div class="nav-links">
        <a href="/mainnet">Mainnet</a>
        <a href="/live">Status</a>
        <a href="/monitor">Monitor</a>
        <a href="/launch">Restricted launch</a>
        <a href="/transparency">Transparency</a>
        <a href="/roadmap">Roadmap</a>
      </div>
    </nav>
  </div>

  <main class="page">
    <section class="hero">
      <div>
        <div class="eyebrow">
          <span class="dot"></span>
          Base Mainnet · restricted operation
        </div>

        <h1>Governed treasury infrastructure for monitored, human-approved workflows.</h1>

        <p class="hero-copy">
          AstraTreasury combines AI-assisted signal generation, policy validation, Safe-based governance,
          and public monitoring. The Base Mainnet deployment is intentionally restricted while future
          capabilities follow separate legal, security, governance, and operations approval tracks.
        </p>

        <div class="cta-row">
          <a class="button primary" href="/launch">View restricted launch status</a>
          <a class="button" href="/mainnet">View contracts</a>
          <a class="button" href="/transparency">Transparency index</a>
        </div>
      </div>

      <aside class="hero-card" aria-label="Operational snapshot">
        <div class="snapshot-header">
          <div>
            <h2>Operational snapshot</h2>
            <p>Read-only public status for the restricted Base Mainnet deployment.</p>
          </div>
          <div class="badge">Restricted</div>
        </div>

        <div class="status-table">
          <div class="status-row">
            <div class="status-label">Launch status</div>
            <div class="status-value ${statusTone(rawLaunchStatus)}">${escapeHtml(humanizeStatus(rawLaunchStatus))}</div>
          </div>

          <div class="status-row">
            <div class="status-label">Mainnet mode</div>
            <div class="status-value ${statusTone(rawLiveStatus)}">${escapeHtml(humanizeStatus(rawLiveStatus))}</div>
          </div>

          <div class="status-row">
            <div class="status-label">Monitor</div>
            <div class="status-value ${statusTone(rawMonitorStatus)}">${escapeHtml(humanizeStatus(rawMonitorStatus))}</div>
          </div>

          <div class="status-row">
            <div class="status-label">Alerts & incidents</div>
            <div class="status-value ${alertTone}">${escapeHtml(alertDisplay)}</div>
          </div>

          <div class="status-row">
            <div class="status-label">Stabilization</div>
            <div class="status-value ${statusTone(rawStabilizationStatus)}">${escapeHtml(humanizeStatus(rawStabilizationStatus))}</div>
          </div>
        </div>

        <div class="disclosure">
          No public token sale, real treasury funding, staking, rewards, buybacks, autonomous execution,
          or mainnet execution queue activation is approved.
        </div>
      </aside>
    </section>

    <section class="section">
      <div class="section-head">
        <h2>Restricted by design</h2>
        <p>
          AstraTreasury is structured to separate deployment visibility from operational activation.
          Mainnet contracts are public, but sensitive capabilities remain disabled until separately approved.
        </p>
      </div>

      <div class="grid-3">
        <div class="panel">
          <div class="panel-kicker">Governance</div>
          <h3>Safe-based controls</h3>
          <p>Governance setup is routed through Safe-controlled approval flows rather than single-operator authority.</p>
        </div>

        <div class="panel">
          <div class="panel-kicker">Monitoring</div>
          <h3>Public observability</h3>
          <p>Operational, event, alert, incident, evidence, and launch status pages are available as public read-only records.</p>
        </div>

        <div class="panel">
          <div class="panel-kicker">Restrictions</div>
          <h3>Capabilities disabled</h3>
          <p>Execution queue, autonomous execution, funding, rewards, buybacks, and public sale activity remain off.</p>
        </div>
      </div>
    </section>

    <section class="section">
      <div class="section-head">
        <h2>Base Mainnet contracts</h2>
        <p>Public contract references for the restricted mainnet deployment.</p>
      </div>

      <div class="contracts">
        <table>
          <thead>
            <tr>
              <th>Contract</th>
              <th>Address</th>
              <th>Explorer</th>
            </tr>
          </thead>
          <tbody>
            ${contractRows || '<tr><td colspan="3">No contracts found in the public manifest yet.</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>

    <section class="section">
      <div class="section-head">
        <h2>Public record</h2>
        <p>Sanitized public pages for review, monitoring, evidence, and future capability governance.</p>
      </div>

      <div class="link-grid">
        <a class="link-card" href="/trust">
          <strong>Trust Center</strong>
          <span>Deployment, monitoring, alerts, incidents, token metadata, and restrictions.</span>
        </a>
        <a class="link-card" href="/launch-control">
          <strong>Launch Control</strong>
          <span>Consolidated launch matrix. Generic prep is complete.</span>
        </a>
        <a class="link-card" href="/capability-matrix">
          <strong>Capability Matrix</strong>
          <span>Finalized all-disabled capability map.</span>
        </a>
        <a class="link-card" href="/public-status-update">
          <strong>Public Status Update</strong>
          <span>Finalized restricted-mode public status language.</span>
        </a>
        <a class="link-card" href="/governance-decision-approval">
          <strong>Governance decision approval</strong>
          <span>First action-specific approval path. Approval is not recorded.</span>
        </a>
        <a class="link-card" href="/governance-decision-live-precheck">
          <strong>Decision live precheck</strong>
          <span>Final precheck before decision recording.</span>
        </a>
        <a class="link-card" href="/governance-decision">
          <strong>Governance decision</strong>
          <span>Restricted-mode / all-disabled decision record.</span>
        </a>
        <a class="link-card" href="/restricted-mode-final-status">
          <strong>Final restricted-mode status</strong>
          <span>Synced public status after governance decision recording.</span>
        </a>
        <a class="link-card" href="/restricted-mode-monitoring-baseline">
          <strong>Monitoring baseline</strong>
          <span>Post-governance restricted-mode monitoring snapshot.</span>
        </a>
        <a class="link-card" href="/restricted-mode-evidence-seal">
          <strong>Evidence seal</strong>
          <span>Cryptographic evidence manifest for restricted-mode posture.</span>
        </a>
        <a class="link-card" href="/restricted-mode-release-candidate">
          <strong>Release candidate</strong>
          <span>Final restricted-mode release candidate package.</span>
        </a>
        <a class="link-card" href="/restricted-mode-final-release">
          <strong>Final release</strong>
          <span>Restricted-mode final release package.</span>
        </a>
        <a class="link-card" href="/restricted-mode-operations-handoff">
          <strong>Operations handoff</strong>
          <span>Restricted-mode steady-state operating procedure.</span>
        </a>
        <a class="link-card" href="/restricted-mode-maintenance-schedule">
          <strong>Maintenance schedule</strong>
          <span>Recurring restricted-mode maintenance cadence.</span>
        </a>
        <a class="link-card" href="/restricted-mode-operator-checklist">
          <strong>Operator checklist</strong>
          <span>Practical controls before production or status changes.</span>
        </a>
        <a class="link-card" href="/capability-activation-intake">
          <strong>Capability intake</strong>
          <span>Safe request gate for future capability activation.</span>
        </a>
        <a class="link-card" href="/capability-request-import">
          <strong>Capability request import</strong>
          <span>Guarded request import for review only.</span>
        </a>
        <a class="link-card" href="/capability-request-review">
          <strong>Capability request review</strong>
          <span>Review gate before any dedicated action path.</span>
        </a>
        <a class="link-card" href="/dex-liquidity-path">
          <strong>DEX/liquidity path</strong>
          <span>Selected public purchase path. Not approved.</span>
        </a>
        <a class="link-card" href="/dex-liquidity-parameters">
          <strong>DEX liquidity parameters</strong>
          <span>Parameter review. Not finalized.</span>
        </a>
        <a class="link-card" href="/dex-liquidity-parameter-selection">
          <strong>DEX parameter selection</strong>
          <span>Selection import for review only.</span>
        </a>
        <a class="link-card" href="/dex-liquidity-parameter-finalization">
          <strong>DEX parameter finalization</strong>
          <span>Review readiness. Not approved.</span>
        </a>
        <a class="link-card" href="/dex-liquidity-parameter-approval">
          <strong>DEX parameter approval</strong>
          <span>Parameter-only approval. No pool or liquidity.</span>
        </a>
        <a class="link-card" href="/dex-liquidity-source-safe-impact">
          <strong>DEX source/Safe impact</strong>
          <span>Planning approval only. No funds or Safe payload.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-readiness">
          <strong>DEX pool readiness</strong>
          <span>Ready for later pool-creation approval. No pool created.</span>
        </a>
        <a class="link-card" href="/dex-pool-existence-precheck">
          <strong>DEX pool existence precheck</strong>
          <span>Read-only factory check. No pool created.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-approval">
          <strong>DEX pool creation approval</strong>
          <span>Path approval only. No pool created.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-execution-precheck">
          <strong>DEX execution precheck</strong>
          <span>Routes to Safe or direct preparation. No execution.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-payload-preparation">
          <strong>DEX Safe payload preparation</strong>
          <span>Safe path ready. No payload generated.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-payload-draft">
          <strong>DEX Safe payload draft</strong>
          <span>Non-executable draft. No calldata or payload.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-payload-draft-review">
          <strong>DEX Safe payload draft review</strong>
          <span>Token ordering review. No payload generated.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-token-ordering-sqrtprice">
          <strong>DEX sqrtPriceX96 review</strong>
          <span>Token ordering and price calculation. No payload.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-factory-router-review">
          <strong>DEX factory/router review</strong>
          <span>Execution path review. No calldata or payload.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-owners-threshold">
          <strong>DEX Safe owners/threshold</strong>
          <span>Safe account review. No payload generated.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-payload-generation-approval">
          <strong>DEX Safe payload generation approval</strong>
          <span>Generation approved. No payload generated yet.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-payload-generation">
          <strong>DEX Safe payload generation</strong>
          <span>Local payload generated. Not submitted or executed.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-payload-verification">
          <strong>DEX Safe payload verification</strong>
          <span>Payload verified. Not submitted or executed.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-submission-approval">
          <strong>DEX Safe submission approval</strong>
          <span>Submission approved. Not submitted or executed.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-submission-preparation">
          <strong>DEX Safe submission preparation</strong>
          <span>Candidate prepared. Not submitted or executed.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-submission-dry-run">
          <strong>DEX Safe submission dry run</strong>
          <span>Operator command review. Not submitted.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-submission-execution-approval">
          <strong>DEX Safe submission execution approval</strong>
          <span>Submission action approved. Not submitted.</span>
        </a>
        <a class="link-card" href="/dex-pool-creation-safe-submission-live">
          <strong>DEX Safe submission live</strong>
          <span>Submitted/pending. Not executed.</span>
        </a>
        <a class="link-card" href="/governance-vote-result-evidence">
          <strong>Vote/result evidence</strong>
          <span>Evidence import path. No decision is recorded.</span>
        </a>
        <a class="link-card" href="/signed-governance-resolution-evidence">
          <strong>Signed resolution evidence</strong>
          <span>Evidence import path. No decision is recorded.</span>
        </a>
        <a class="link-card" href="/protocol">
          <strong>Protocol</strong>
          <span>How AstraTreasury works at a high level.</span>
        </a>
        <a class="link-card" href="/architecture">
          <strong>Architecture</strong>
          <span>Contracts, roles, and control model.</span>
        </a>
        <a class="link-card" href="/security">
          <strong>Security</strong>
          <span>Monitoring, alerts, incidents, and restricted controls.</span>
        </a>
        <a class="link-card" href="/live">
          <strong>Live status</strong>
          <span>Current restricted mainnet posture.</span>
        </a>
        <a class="link-card" href="/monitor">
          <strong>Mainnet monitor</strong>
          <span>Read-only operational checks.</span>
        </a>
        <a class="link-card" href="/alerts">
          <strong>Alerts</strong>
          <span>Alert escalation status.</span>
        </a>
        <a class="link-card" href="/incidents">
          <strong>Incidents</strong>
          <span>Sanitized incident summary.</span>
        </a>
        <a class="link-card" href="/evidence">
          <strong>Evidence</strong>
          <span>Historical public snapshots.</span>
        </a>
        <a class="link-card" href="/announcement">
          <strong>Announcement</strong>
          <span>Restricted launch announcement materials.</span>
        </a>
        <a class="link-card" href="/roadmap">
          <strong>Roadmap</strong>
          <span>Future capabilities and approval tracks.</span>
        </a>
        <a class="link-card" href="/market">
          <strong>Market data</strong>
          <span>Read-only DEX market status when public data exists.</span>
        </a>
        <a class="link-card" href="/wallet">
          <strong>Add to wallet</strong>
          <span>Wallet visibility for ASTP. This is not a purchase flow.</span>
        </a>
        <a class="link-card" href="/faq">
          <strong>FAQ</strong>
          <span>Plain-language answers about restricted mode and ASTP.</span>
        </a>
        <a class="link-card" href="/api">
          <strong>API directory</strong>
          <span>Read-only public JSON endpoints.</span>
        </a>
        <a class="link-card" href="/api/public/home">
          <strong>Home API</strong>
          <span>Machine-readable public status.</span>
        </a>
      </div>
    </section>

    <footer class="footer">
      AstraTreasury is not offering a public token sale and is not marketed as an investment product.
      The restricted Base Mainnet deployment does not approve real treasury funding, staking, rewards,
      buybacks, autonomous execution, or mainnet execution queue activation.
      <br />
      Generated ${escapeHtml(status.generatedAt)} · Commit ${escapeHtml(shortCommit)}
    </footer>
  </main>
</body>
</html>`;

fs.writeFileSync(outHtml, html + "\n");

console.log("Wrote public-docs/index.html");
console.log("Wrote public-docs/home-status.json");
