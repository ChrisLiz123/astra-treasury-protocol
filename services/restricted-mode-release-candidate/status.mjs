...
const reportDir = path.join(root, "reports", "restricted-mode-release-candidate");
const reportFile = path.join(reportDir, "restricted-mode-release-candidate-status.json");

const publicJsonFile = path.join(root, "public-docs", "restricted-mode-release-candidate-status.json");
const publicHtmlFile = path.join(root, "public-docs", "restricted-mode-release-candidate.html");
...
const config = readJson("configs/restricted-mode-release-candidate.config.json");
...
const missingArtifacts = releaseArtifactPaths.filter((artifactPath) => {
  return !fs.existsSync(path.join(root, artifactPath));
});

for (const artifactPath of missingArtifacts) {
  check(checks, `Release artifact exists: ${artifactPath}`, false, { artifactPath });
}

const artifactHashes = releaseArtifactPaths
  .filter((artifactPath) => fs.existsSync(path.join(root, artifactPath)))
  .map((artifactPath) => sha256File(artifactPath));

const candidatePayload = {
  schema: "astra-restricted-mode-release-candidate-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  releaseCandidateScope: "post-governance-decision-restricted-mode-all-disabled",
  artifactHashes
};

const releaseCandidateHash = sha256Json(candidatePayload);
const failures = checks.filter((item) => !item.pass);

const status = failures.length === 0
  ? "RESTRICTED_MODE_RELEASE_CANDIDATE_READY_DECISION_RECORDED_ALL_DISABLED"
  : "RESTRICTED_MODE_RELEASE_CANDIDATE_REVIEW_REQUIRED";

const report = {
  schema: "astra-restricted-mode-release-candidate-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  releaseCandidateScope: config.releaseCandidateScope || "post-governance-decision-restricted-mode-all-disabled",
  publicStatement:
    "AstraTreasury restricted-mode release candidate is ready. Governance decision is recorded, all restricted capabilities remain disabled and not approved, and evidence is sealed.",
  summary: {
...
<head>
  ...
  <title>AstraTreasury Restricted-Mode Release Candidate</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Release candidate · restricted mode</div>
    <h1>Restricted-Mode Release Candidate</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Release candidate hash:</strong> <code>${escapeHtml(releaseCandidateHash)}</code></p>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This release candidate is a restricted-mode package only. It does not approve launch, funding,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/restricted-mode-release-candidate">/api/public/restricted-mode-release-candidate</a></p>
    <p><a href="/restricted-mode-evidence-seal">Restricted-Mode Evidence Seal</a></p>
    <p><a href="/restricted-mode-monitoring-baseline">Restricted-Mode Monitoring Baseline</a></p>
...
console.log("AstraTreasury Restricted-Mode Release Candidate");
console.log("===============================================");
console.log(`Status: ${report.status}`);
console.log(`Release artifacts: ${report.summary.artifactCount}`);
console.log(`Release candidate hash: ${releaseCandidateHash}`);
