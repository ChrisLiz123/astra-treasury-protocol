...
const reportDir = path.join(root, "reports", "dex-liquidity-funding-transfer-safe-payload-verification");
const statusReportFile = path.join(reportDir, "dex-liquidity-funding-transfer-safe-payload-verification-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-payload-verification-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-payload-verification.html");
...
const verificationPresent = Boolean(verification && verification.schema === "astra-dex-liquidity-funding-transfer-safe-payload-verification-v0.1");
...
const status = verificationPresent
  ? verification.status
  : "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_PAYLOAD_VERIFICATION_NOT_RUN";
...
const checkRows = (verification?.transactionChecks || []).map((item) => {
  return `<tr>
    <td>${escapeHtml(item.role)}</td>
    <td>${escapeHtml(item.symbol)}</td>
    <td><code>${escapeHtml(item.tokenAddress)}</code></td>
    <td>${escapeHtml(item.amountMatchesShortfall)}</td>
    <td>${escapeHtml(item.sourceBalanceCoversTransfer)}</td>
    <td>${escapeHtml(item.destinationBalanceUnchanged)}</td>
  </tr>`;
}).join("");

const publicPayload = {
  schema: "astra-dex-liquidity-funding-transfer-safe-payload-verification-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: status === "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_PAYLOAD_VERIFIED_NOT_SUBMITTED_NO_FUNDS_MOVED"
    ? "AstraTreasury verified the generated DEX liquidity funding-transfer Safe payload. The payload has not been submitted, no Safe transaction has executed, no treasury funds have moved, no token approvals have executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity funding-transfer Safe payload verification has not completed successfully.",
  summary: {
    verificationPresent,
    fundingTransferSafePayloadVerified: status === "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_PAYLOAD_VERIFIED_NOT_SUBMITTED_NO_FUNDS_MOVED",
    payloadHashVerified: verification?.payloadHashVerified === true,
    payloadHash: verification?.payloadHash || "",
    sourceSafeAddress: verification?.sourceSafeAddress || "",
    destinationSafeAddress: verification?.destinationSafeAddress || "",
    transactionCount: verification?.transactionCount || 0,
    ...
    nextRecommendedMilestone: "DEX Liquidity Treasury Funding Transfer Safe Submission Approval",
    artifactCount: artifactHashes.length
  },
  fundingTransferSafePayloadVerification: {
    verificationHash: verification?.verificationHash || "",
    hashAlgorithm: "SHA-256",
    payloadReference: verification?.payloadReference || "",
    payloadHash: verification?.payloadHash || "",
    recomputedPayloadHash: verification?.recomputedPayloadHash || "",
    transactionChecks: verification?.transactionChecks || [],
    requiredBeforeFundingTransferSubmissionApproval: verification?.requiredBeforeFundingTransferSubmissionApproval || {},
    artifactHashes
  },
  currentStatuses: {
    dexFundingTransferSafePayload: payloadStatus.status || "UNKNOWN",
    dexFundingTransferPayloadGenerationApproval: payloadApproval.status || "UNKNOWN",
    dexFundingTransferApproval: transferApproval.status || "UNKNOWN",
    dexFundingTransferRequirements: requirementsStatus.status || "UNKNOWN",
...
const requiredRows = Object.entries(publicPayload.fundingTransferSafePayloadVerification.requiredBeforeFundingTransferSubmissionApproval || {}).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Funding Transfer Safe Payload Verification</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Funding-transfer payload verified · not submitted · no funds moved</div>
    <h1>DEX Funding Transfer Safe Payload Verification</h1>
    <p>${escapeHtml(publicPayload.publicStatement)}</p>
    <p><strong>Verification hash:</strong> <code>${escapeHtml(publicPayload.fundingTransferSafePayloadVerification.verificationHash)}</code></p>
  </section>
  ...
  <section class="card">
    <h2>Transaction verification checks</h2>
    <table>
      <thead><tr><th>Role</th><th>Symbol</th><th>Token</th><th>Amount matches shortfall</th><th>Source balance covers transfer</th><th>Destination unchanged</th></tr></thead>
      <tbody>${checkRows || '<tr><td colspan="6">No verification checks available.</td></tr>'}</tbody>
    </table>
  </section>

  <section class="card">
    <h2>Required before submission approval</h2>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This is verification only. It has not submitted or executed the Safe transaction, and no funds have moved.
      Safe submission and execution require separate future approvals.
    </div>
  </section>

  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-funding-transfer-safe-payload-verification">/api/public/dex-liquidity-funding-transfer-safe-payload-verification</a></p>
    <p><a href="/dex-liquidity-funding-transfer-safe-payload">DEX Funding Transfer Safe Payload</a></p>
    <p><a href="/dex-liquidity-funding-transfer-payload-generation-approval">DEX Funding Transfer Payload Generation Approval</a></p>
...
console.log("AstraTreasury DEX Funding Transfer Safe Payload Verification");
console.log("===========================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Payload verified: ${publicPayload.summary.fundingTransferSafePayloadVerified}`);
