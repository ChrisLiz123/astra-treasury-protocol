...
const reportFile = path.join(reportDir, "dex-pool-creation-safe-submission-dry-run-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-pool-creation-safe-submission-dry-run-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-pool-creation-safe-submission-dry-run.html");
...
const reviewPresent = Boolean(review && review.schema === "astra-dex-pool-creation-safe-submission-dry-run-review-v0.1");
...
const status = reviewPresent
  ? review.status
  : "DEX_POOL_CREATION_SAFE_SUBMISSION_DRY_RUN_REVIEW_NOT_RUN";

const artifactPaths = [
  reviewRelativePath,
  "reports/dex-pool-creation-safe-submission-preparation/dex-pool-creation-safe-submission-preparation.json",
...
const report = {
  ...
  publicStatement: status === "DEX_POOL_CREATION_SAFE_SUBMISSION_DRY_RUN_REVIEW_READY_NOT_SUBMITTED"
    ? "AstraTreasury completed the DEX pool creation Safe submission dry run and operator command review. No Safe API call was made, the Safe UI was not opened by automation, the transaction was not submitted, not queued, not executed, no pool is created, no liquidity is added, no funds are moved, and public trading is not approved."
    : "AstraTreasury DEX pool creation Safe submission dry run has not completed successfully.",
  summary: {
    dryRunReady: status === "DEX_POOL_CREATION_SAFE_SUBMISSION_DRY_RUN_REVIEW_READY_NOT_SUBMITTED",
    ...
    dataHash: review?.safeSubmissionCandidateReview?.dataHash || "",
  ...
  dryRunReview: {
    ...
    candidateReview: review?.safeSubmissionCandidateReview || {},
    operatorCommandReview: review?.operatorCommandReview || {},
    requiredBeforeActualSafeSubmission: review?.requiredBeforeActualSafeSubmission || {},
    artifactHashes
  },
  currentStatuses: {
...
const candidateRows = Object.entries(report.dryRunReview.candidateReview).map(([key, value]) => {
  return `<tr><td>${escapeHtml(key)}</td><td>${escapeHtml(typeof value === "object" ? JSON.stringify(value) : value)}</td></tr>`;
}).join("");

const commandRows = Object.entries(report.dryRunReview.operatorCommandReview).map(([key, value]) => {
  return `<tr><td>${escapeHtml(key)}</td><td>${escapeHtml(Array.isArray(value) ? value.join(" | ") : typeof value === "object" ? JSON.stringify(value) : value)}</td></tr>`;
}).join("");

const requiredRows = Object.entries(report.dryRunReview.requiredBeforeActualSafeSubmission).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Safe Submission Dry Run</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Safe submission dry run · not submitted</div>
    <h1>DEX Safe Submission Dry Run</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Dry run hash:</strong> <code>${escapeHtml(report.dryRunReview.dryRunReviewHash)}</code></p>
  </section>
  ...
  <section class="card">
    ...
    <table>
      <thead><tr><th>Field</th><th>Value</th></tr></thead>
      <tbody>${candidateRows || '<tr><td colspan="2">No candidate review available.</td></tr>'}</tbody>
    </table>
  </section>

  <section class="card">
    <h2>Operator command review</h2>
    <table>
      <thead><tr><th>Field</th><th>Value</th></tr></thead>
      <tbody>${commandRows || '<tr><td colspan="2">No operator review available.</td></tr>'}</tbody>
    </table>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This page completes a dry run and command review only. It does not call Safe APIs,
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-pool-creation-safe-submission-dry-run">/api/public/dex-pool-creation-safe-submission-dry-run</a></p>
...
console.log("AstraTreasury DEX Safe Submission Dry Run");
console.log("=========================================");
console.log(`Status: ${report.status}`);
console.log(`Dry run ready: ${report.summary.dryRunReady}`);
console.log(`Safe API called: ${report.summary.callsSafeTransactionService}`);
