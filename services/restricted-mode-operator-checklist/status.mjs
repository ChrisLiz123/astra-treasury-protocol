...
const reportDir = path.join(root, "reports", "restricted-mode-operator-checklist");
const reportFile = path.join(reportDir, "restricted-mode-operator-checklist-status.json");

const publicJsonFile = path.join(root, "public-docs", "restricted-mode-operator-checklist-status.json");
const publicHtmlFile = path.join(root, "public-docs", "restricted-mode-operator-checklist.html");

fs.mkdirSync(reportDir, { recursive: true });

const checklistArtifactPaths = [
  ...
  "docs/restricted-mode-operator-checklist/RESTRICTED_MODE_OPERATOR_CHECKLIST.md",
  "docs/restricted-mode-operator-checklist/BEFORE_TOUCHING_PRODUCTION.md",
  "docs/restricted-mode-operator-checklist/PUBLIC_STATUS_CHANGE_CHECKLIST.md",
  "docs/restricted-mode-operator-checklist/FUTURE_CAPABILITY_APPROVAL_START_CHECKLIST.md",
  "docs/restricted-mode-operator-checklist/INCIDENT_OPERATOR_CHECKLIST.md"
];
...
const config = readJson("configs/restricted-mode-operator-checklist.config.json");
...
const missingArtifacts = checklistArtifactPaths.filter((artifactPath) => {
  return !fs.existsSync(path.join(root, artifactPath));
});

for (const artifactPath of missingArtifacts) {
  check(checks, `Checklist artifact exists: ${artifactPath}`, false, { artifactPath });
}

const artifactHashes = checklistArtifactPaths
  .filter((artifactPath) => fs.existsSync(path.join(root, artifactPath)))
  .map((artifactPath) => sha256File(artifactPath));

const checklistPayload = {
  schema: "astra-restricted-mode-operator-checklist-payload-v0.1",
  currentApprovedMode: "restricted-mainnet-operation",
  operatorChecklistScope: "post-maintenance-schedule-restricted-mode-all-disabled",
  artifactHashes
};

const operatorChecklistHash = sha256Json(checklistPayload);
const failures = checks.filter((item) => !item.pass);

const status = failures.length === 0
  ? "RESTRICTED_MODE_OPERATOR_CHECKLIST_READY_DECISION_RECORDED_ALL_DISABLED"
  : "RESTRICTED_MODE_OPERATOR_CHECKLIST_REVIEW_REQUIRED";

const report = {
  schema: "astra-restricted-mode-operator-checklist-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  operatorChecklistScope: config.operatorChecklistScope || "post-maintenance-schedule-restricted-mode-all-disabled",
  publicStatement:
    "AstraTreasury restricted-mode operator checklist is ready. Governance decision is recorded, restricted-mode operations are in steady state, and all restricted capabilities remain disabled and not approved.",
  summary: {
  ...
  operatorChecklist: {
    ...
    checklist: config.operatorChecklist || {},
    artifactHashes
  },
  checks,
  failures,
...
const checklistRows = Object.entries(report.operatorChecklist.checklist).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury Restricted-Mode Operator Checklist</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    <div class="badge">Operator checklist · restricted mode</div>
    <h1>Restricted-Mode Operator Checklist</h1>
    <p>${escapeHtml(report.publicStatement)}</p>
    <p><strong>Operator checklist hash:</strong> <code>${escapeHtml(operatorChecklistHash)}</code></p>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This operator checklist documents restricted-mode operational controls only.
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/restricted-mode-operator-checklist">/api/public/restricted-mode-operator-checklist</a></p>
    <p><a href="/restricted-mode-maintenance-schedule">Restricted-Mode Maintenance Schedule</a></p>
...
console.log("AstraTreasury Restricted-Mode Operator Checklist");
console.log("================================================");
console.log(`Status: ${report.status}`);
console.log(`Checklist artifacts: ${report.summary.artifactCount}`);
console.log(`Operator checklist hash: ${operatorChecklistHash}`);
