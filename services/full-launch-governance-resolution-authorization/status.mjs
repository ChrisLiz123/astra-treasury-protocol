...
const files = {
  config: "configs/full-launch-governance-resolution-authorization.config.json",
...
const reportDir = path.join(root, "reports", "full-launch-governance-resolution-authorization");
const reportFile = path.join(reportDir, "full-launch-governance-resolution-authorization-status.json");

const publicJsonFile = path.join(root, "public-docs", "full-launch-governance-resolution-authorization-status.json");
const publicHtmlFile = path.join(root, "public-docs", "full-launch-governance-resolution-authorization.html");
...
check(checks, "Resolution signing authorization package prepared", config.resolutionSigningAuthorizationPackagePrepared === true, {
  resolutionSigningAuthorizationPackagePrepared: config.resolutionSigningAuthorizationPackagePrepared
});
...
check(checks, "Resolution signing authorization not requested", config.resolutionSigningAuthorizationRequested === false, {
  resolutionSigningAuthorizationRequested: config.resolutionSigningAuthorizationRequested
});

check(checks, "Resolution signing authorization not recorded", config.resolutionSigningAuthorizationRecorded === false, {
  resolutionSigningAuthorizationRecorded: config.resolutionSigningAuthorizationRecorded
});
...
check(checks, "Draft authorization has no resolution hash", !record.resolutionHash, {
...
const dryRunCases = [
  ...
  {
    id: "RESOLUTION-AUTH-003",
    title: "Missing final resolution text blocks authorization",
    expected: "BLOCKED",
    actual: config.requiredBeforeResolutionSigningAuthorization?.resolutionTextFinalApproved === false ? "BLOCKED" : "NOT_BLOCKED",
    reason: "resolution text is not final-approved"
  },
  {
    id: "RESOLUTION-AUTH-004",
    title: "Missing capability matrix blocks authorization",
    expected: "BLOCKED",
    actual: config.requiredBeforeResolutionSigningAuthorization?.capabilityMatrixFinalApproved === false ? "BLOCKED" : "NOT_BLOCKED",
    reason: "capability matrix is not final-approved"
  },
  {
    id: "RESOLUTION-AUTH-005",
    title: "Missing signer list blocks authorization",
    expected: "BLOCKED",
    actual: config.requiredBeforeResolutionSigningAuthorization?.signerListConfirmed === false ? "BLOCKED" : "NOT_BLOCKED",
    reason: "signer list is not confirmed"
  },
  {
    id: "RESOLUTION-AUTH-006",
...
const authorizationStatus = {
  resolutionSigningAuthorizationPackagePrepared: Boolean(config.resolutionSigningAuthorizationPackagePrepared),
  authorizationEvidencePlanPrepared: Boolean(config.authorizationEvidencePlanPrepared),
  resolutionSigningAuthorizationRequested: Boolean(config.resolutionSigningAuthorizationRequested),
...
const status = failures.length === 0
  ? "FULL_LAUNCH_GOVERNANCE_RESOLUTION_AUTHORIZATION_PACKAGE_READY_NOT_RECORDED"
  : "FULL_LAUNCH_GOVERNANCE_RESOLUTION_AUTHORIZATION_REVIEW_REQUIRED";

const report = {
  schema: "astra-full-launch-governance-resolution-authorization-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  mode: "PLANNING_ONLY_RESOLUTION_SIGNING_AUTHORIZATION_NOT_RECORDED",
  currentApprovedMode: "restricted-mainnet-operation",
  resolutionSigningAuthorizationRecorded: false,
  resolutionSigningAuthorized: false,
  ...
  publicStatement:
    "AstraTreasury governance resolution signing authorization package is ready for review. Resolution signing is not authorized, no resolution is signed, no governance decision is recorded, and full launch is not approved.",
  summary: {
  ...
  requiredBeforeResolutionSigningAuthorization: config.requiredBeforeResolutionSigningAuthorization || {},
  ...
  nextPossibleMilestones: [
    "Governance resolution signing live package",
    "Governance decision recording package",
  ...
  safety: {
    ...
    authorizesResolutionSigning: false,
...
writeJson(publicJsonFile, {
  schema: "astra-public-full-launch-governance-resolution-authorization-status-v0.1",
  generatedAt: report.generatedAt,
  status: report.status,
  mode: report.mode,
  currentApprovedMode: report.currentApprovedMode,
  resolutionSigningAuthorizationRecorded: report.resolutionSigningAuthorizationRecorded,
  resolutionSigningAuthorized: report.resolutionSigningAuthorized,
  ...
  requiredBeforeResolutionSigningAuthorization: report.requiredBeforeResolutionSigningAuthorization,
...
<head>
  ...
  <title>AstraTreasury Governance Resolution Authorization</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #08111f;
...
<main>
  <section class="card">
    ...
    <h1>Governance Resolution Authorization</h1>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      Resolution signing authorization is not recorded. No governance resolution is signed. No governance decision is recorded.
  ...
  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/full-launch-governance-resolution-authorization">/api/public/full-launch-governance-resolution-authorization</a></p>
...
console.log("AstraTreasury Governance Resolution Authorization");
console.log("=================================================");
