...
const reportDir = path.join(root, "reports", "dex-liquidity-funding-transfer-safe-submission-dry-run");
const statusReportFile = path.join(reportDir, "dex-liquidity-funding-transfer-safe-submission-dry-run-status.json");

const publicJsonFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-submission-dry-run-status.json");
const publicHtmlFile = path.join(root, "public-docs", "dex-liquidity-funding-transfer-safe-submission-dry-run.html");
...
const recordPresent = Boolean(record && record.schema === "astra-dex-liquidity-funding-transfer-safe-submission-dry-run-v0.1");
...
const status = recordPresent
  ? record.status
  : "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_SUBMISSION_DRY_RUN_NOT_RUN";
...
const publicPayload = {
  schema: "astra-dex-liquidity-funding-transfer-safe-submission-dry-run-status-v0.1",
  generatedAt: new Date().toISOString(),
  status,
  currentApprovedMode: "restricted-mainnet-operation",
  publicStatement: status === "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_SUBMISSION_DRY_RUN_COMPLETE_NOT_SUBMITTED_NO_FUNDS_MOVED"
    ? "AstraTreasury completed a dry run of the DEX liquidity funding-transfer Safe submission package. The Safe transaction has not been submitted, no Safe transaction has executed, no treasury funds have moved, no token approvals have executed, no liquidity has been added, public trading is not approved, and full launch is not approved."
    : "AstraTreasury DEX liquidity funding-transfer Safe submission dry run has not completed successfully.",
  summary: {
    recordPresent,
    safeSubmissionDryRunComplete: status === "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_SAFE_SUBMISSION_DRY_RUN_COMPLETE_NOT_SUBMITTED_NO_FUNDS_MOVED",
    ...
    nextRecommendedMilestone: "DEX Liquidity Treasury Funding Transfer Safe Submission Live",
    artifactCount: artifactHashes.length
  },
  safeSubmissionDryRun: {
    ...
    requiredBeforeFundingTransferSafeSubmissionLive: record?.requiredBeforeFundingTransferSafeSubmissionLive || {},
    artifactHashes
  },
  currentStatuses: {
...
const requiredRows = Object.entries(publicPayload.safeSubmissionDryRun.requiredBeforeFundingTransferSafeSubmissionLive || {}).map(([key, value]) => {
...
<head>
  ...
  <title>AstraTreasury DEX Funding Transfer Safe Submission Dry Run</title>
  <style>
...
<main>
  <section class="card">
    <div class="badge">Safe submission dry run · not submitted · no funds moved</div>
    <h1>DEX Funding Transfer Safe Submission Dry Run</h1>
    <p>${escapeHtml(publicPayload.publicStatement)}</p>
    <p><strong>Dry-run hash:</strong> <code>${escapeHtml(publicPayload.safeSubmissionDryRun.dryRunHash)}</code></p>
  </section>
  ...
  <section class="card">
    ...
    <table>
      <thead><tr><th>#</th><th>Role</th><th>Symbol</th><th>Builder matches payload</th><th>Source balance covers transfer</th><th>Destination unchanged</th></tr></thead>
      <tbody>${checkRows || '<tr><td colspan="6">No dry-run checks.</td></tr>'}</tbody>
    </table>
  </section>
  ...
  <section class="card">
    <h2>Important</h2>
    <div class="notice">
      This is a dry run only. It has not submitted or executed the Safe transaction, and no funds have moved.
      The next step is the live Safe submission/proposal step, not execution.
    </div>
  </section>

  <section class="card">
    <h2>Public API</h2>
    <p><a href="/api/public/dex-liquidity-funding-transfer-safe-submission-dry-run">/api/public/dex-liquidity-funding-transfer-safe-submission-dry-run</a></p>
...
console.log("AstraTreasury DEX Funding Transfer Safe Submission Dry Run");
console.log("=========================================================");
console.log(`Status: ${publicPayload.status}`);
console.log(`Dry run complete: ${publicPayload.summary.safeSubmissionDryRunComplete}`);
