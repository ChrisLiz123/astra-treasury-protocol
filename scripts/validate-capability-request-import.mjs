...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required capability request import file.");
  }
}
...
if (issues.length === 0) {
  ...
  if (config.requestImportTemplatePrepared !== true) {
    issue("requestImportTemplatePrepared", "Capability request import template must be prepared.");
  }

  if (config.requestImporterPrepared !== true) {
    issue("requestImporterPrepared", "Capability request importer must be prepared.");
  }
  ...
  if (fs.existsSync(requestPath)) {
    ...
    for (const key of [
      ...
      if (request[key] !== false) {
        issue(`request.${key}`, "Request import must not request immediate activation, payload generation, execution, or funds movement.");
      }
    }
  }
}
...
const result = {
  schema: "astra-capability-request-import-validation-v0.1",
