...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required DEX liquidity parameter review file.");
  }
}
...
if (issues.length === 0) {
  ...
  if (config.parameterReviewPrepared !== true) {
    issue("parameterReviewPrepared", "Parameter review framework should be prepared.");
  }

  if (config.parameterReviewOnly !== true) {
    issue("parameterReviewOnly", "This milestone must remain parameter review only.");
  }

  if (config.parametersFinalized !== false) {
    issue("parametersFinalized", "Parameters must remain not finalized.");
  }
  ...
  for (const [key, value] of Object.entries(config.parameterDecisions || {})) {
    if (value !== false) {
      issue(`parameterDecisions.${key}`, "Parameter decision must remain pending.");
    }
  }

  for (const [key, value] of Object.entries(config.requiredBeforeFinalization || {})) {
    if (value !== false) {
      issue(`requiredBeforeFinalization.${key}`, "Finalization requirement must remain false until completed.");
    }
  }
...
const result = {
  schema: "astra-dex-liquidity-parameter-review-validation-v0.1",
