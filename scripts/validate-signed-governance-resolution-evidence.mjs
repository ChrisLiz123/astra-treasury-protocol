...
const requiredFiles = [
  "configs/signed-governance-resolution-evidence-import.config.json",
  "docs/action-approvals/signed-governance-resolution-evidence/SIGNED_GOVERNANCE_RESOLUTION_EVIDENCE_IMPORT.md",
  "docs/action-approvals/signed-governance-resolution-evidence/SIGNED_RESOLUTION_EVIDENCE_TEMPLATE.json",
  "docs/action-approvals/signed-governance-resolution-evidence/SIGNED_RESOLUTION_EVIDENCE_IMPORT_CHECKLIST.md",
  "docs/action-approvals/signed-governance-resolution-evidence/SIGNED_RESOLUTION_EVIDENCE_BLOCKERS.md",
...
const governanceProcessMode = config.governanceProcessMode || "vote-or-resolution";
  ...
  if (voteResultEvidence.voteResultImported === true && voteResultEvidence.voteResultValidated !== true) {
    issue("voteResultEvidence.voteResultValidated", "Imported vote/result evidence must be valid.");
  }
  ...
  if (governanceResolution.governanceResolutionSigned !== false) {
    issue("governanceResolution.governanceResolutionSigned", "Governance resolution status page must remain not signed by system state.");
  }

  if (resolutionAuthorization.resolutionSigningAuthorizationRecorded !== false) {
    issue("resolutionAuthorization.resolutionSigningAuthorizationRecorded", "Resolution authorization status page must remain not recorded by system state.");
  }
  ...
  if (fs.existsSync(evidencePath)) {
    ...
    if (evidence) {
      ...
      if (evidence.governanceResolutionSigned !== true) {
        issue("evidence.governanceResolutionSigned", "governanceResolutionSigned must be true.");
      }

      if (evidence.resolutionSigningAuthorizationRecorded !== true) {
        issue("evidence.resolutionSigningAuthorizationRecorded", "resolutionSigningAuthorizationRecorded must be true.");
      }
      ...
      if (!Array.isArray(evidence.approvedCapabilities)) {
        ...
        issue("evidence.approvedCapabilities", "approvedCapabilities must be empty for this all-disabled restricted-mode path.");
      }

      if (!Array.isArray(evidence.disabledCapabilities)) {
        issue("evidence.disabledCapabilities", "disabledCapabilities must be an array.");
      }
...
const result = {
  schema: "astra-signed-governance-resolution-evidence-validation-v0.1",
