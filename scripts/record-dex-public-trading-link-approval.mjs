...
const confirm = process.env.DEX_PUBLIC_TRADING_LINK_APPROVAL_CONFIRM || "";
const approver = process.env.DEX_PUBLIC_TRADING_LINK_APPROVER || "";
const approvalReference = process.env.DEX_PUBLIC_TRADING_LINK_APPROVAL_REFERENCE || "";
const providedTradingLink = process.env.DEX_PUBLIC_TRADING_LINK_URL || "";
const overwrite = process.env.OVERWRITE_DEX_PUBLIC_TRADING_LINK_APPROVAL || "";

const requiredConfirm = "APPROVE_DEX_PUBLIC_TRADING_LINK_ONLY_NOT_BUY_PAGE_OR_FULL_LAUNCH";

const recordDir = path.join(root, "reports", "dex-public-trading-link-approval");
const recordFile = path.join(recordDir, "dex-public-trading-link-approval-record.json");
const configFile = path.join(root, "configs", "dex-public-trading-link-approval.config.json");
...
if (confirm !== requiredConfirm) {
  issue("DEX_PUBLIC_TRADING_LINK_APPROVAL_CONFIRM", `Must equal ${requiredConfirm}.`);
}

requireUsable("DEX_PUBLIC_TRADING_LINK_APPROVER", approver);
requireUsable("DEX_PUBLIC_TRADING_LINK_APPROVAL_REFERENCE", approvalReference);

if (fs.existsSync(recordFile) && overwrite !== "YES") {
  issue(
    "OVERWRITE_DEX_PUBLIC_TRADING_LINK_APPROVAL",
    "Approval record already exists. Set OVERWRITE_DEX_PUBLIC_TRADING_LINK_APPROVAL=YES only if replacing intentionally."
  );
}
...
requireStatus("publicTradingApprovalStatus.status", publicTradingApprovalStatus.status, "DEX_PUBLIC_TRADING_APPROVED_LIQUIDITY_VERIFIED_BUY_PAGE_NOT_ACTIVATED_FULL_LAUNCH_NOT_APPROVED");
requireStatus("publicTradingApproval.status", publicTradingApproval.status, "DEX_PUBLIC_TRADING_APPROVED_LIQUIDITY_VERIFIED_BUY_PAGE_NOT_ACTIVATED_FULL_LAUNCH_NOT_APPROVED");
...
if (publicTradingApproval.publicTradingApprovalRecorded !== true || publicTradingApproval.publicTradingApproved !== true) {
  issue("publicTradingApproval.flags", "Public trading approval must be recorded and approved.");
}
...
if (readiness.publicTradingReadinessReviewComplete !== true && readiness.readyForPublicTradingApproval !== true && readiness.readyForPublicTradingApprovalReview !== true) {
...
const rpcUrl =
  ...
  process.env.DEX_PUBLIC_TRADING_APPROVAL_RPC_URL ||
...
if (!String(rpcUrl || "").startsWith("https://")) {
  issue("DEX_PUBLIC_TRADING_LINK_APPROVAL_RPC_URL", "RPC URL must be available and start with https://.");
}
...
if (issues.length === 0) {
  try {
    ...
    issue("publicTradingLinkApprovalLiveChecks", error.message);
  }
}

const defaultTradingLink = `https://app.uniswap.org/explore/pools/base/${postVerification.poolAddress}`;
const tradingLinkUrl = providedTradingLink || defaultTradingLink;

if (!isHttpUrl(tradingLinkUrl)) {
  issue("DEX_PUBLIC_TRADING_LINK_URL", "Public trading link URL must be an http(s) URL.");
}

if (issues.length > 0) {
  console.log(JSON.stringify({
    schema: "astra-dex-public-trading-link-approval-result-v0.1",
    checkedAt: new Date().toISOString(),
    status: "STOP_DEX_PUBLIC_TRADING_LINK_NOT_APPROVED",
    issues
  }, null, 2));
  process.exit(1);
}
...
const record = {
  schema: "astra-dex-public-trading-link-approval-record-v0.1",
  recordedAt,
  status: "DEX_PUBLIC_TRADING_LINK_APPROVED_PUBLIC_TRADING_APPROVED_BUY_PAGE_NOT_ACTIVATED_FULL_LAUNCH_NOT_APPROVED",
  approvalScope: "dex-public-trading-link-approval-only-not-buy-page-not-full-launch",
  selectedPublicPurchasePath: "dex-liquidity-pool-trading",
  approver,
  approvalReference,
  tradingLinkUrl,
  defaultTradingLinkUsed: providedTradingLink ? false : true,
  ...
  approvalPrerequisites: {
    ...
    dexPublicTradingReadinessReviewComplete: true,
  ...
  requiredBeforeBuyPageActivationApproval: {
    publicTradingApprovalRecorded: true,
    publicTradingApproved: true,
    publicTradingLinkApprovalRecorded: true,
    publicTradingLinkApproved: true,
...
config.status = "dex-public-trading-link-approved-public-trading-approved-buy-page-not-activated-full-launch-not-approved";
config.publicTradingLinkApprovalRecorded = true;
...
config.approvedDexPublicTradingLink = {
  recordedAt,
  approver,
  approvalReference,
  tradingLinkUrl,
  ...
  recordFile: "reports/dex-public-trading-link-approval/dex-public-trading-link-approval-record.json"
};
