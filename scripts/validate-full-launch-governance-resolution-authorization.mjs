...
const configPath = path.join(root, "configs", "full-launch-governance-resolution-authorization.config.json");

const requiredFiles = [
  "docs/full-launch-governance-resolution-authorization/GOVERNANCE_RESOLUTION_SIGNING_AUTHORIZATION_PACKAGE.md",
  "docs/full-launch-governance-resolution-authorization/GOVERNANCE_RESOLUTION_SIGNING_AUTHORIZATION_CHECKLIST.md",
  "docs/full-launch-governance-resolution-authorization/GOVERNANCE_RESOLUTION_SIGNING_AUTHORIZATION_RECORD.md",
  "docs/full-launch-governance-resolution-authorization/GOVERNANCE_RESOLUTION_AUTHORIZATION_EVIDENCE_PLAN.md",
  "docs/full-launch-governance-resolution-authorization/GOVERNANCE_RESOLUTION_AUTHORIZATION_DRY_RUN_CASES.md",
  "docs/full-launch-governance-resolution-authorization/GOVERNANCE_RESOLUTION_AUTHORIZATION_BLOCKERS.md",
  "docs/full-launch-governance-resolution-authorization/GOVERNANCE_RESOLUTION_AUTHORIZATION_PUBLIC_STATUS_TEMPLATE.md",
...
const forbiddenAuthorizationFiles = [
  "reports/full-launch-governance-resolution/resolution-signing-authorization.json",
  ...
  "public-docs/full-launch-governance-resolution-authorization-recorded.json"
];
...
if (!fs.existsSync(configPath)) {
  issue("configs/full-launch-governance-resolution-authorization.config.json", "Missing governance resolution authorization config.");
}

for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required governance resolution authorization file.");
  }
}

for (const file of forbiddenAuthorizationFiles) {
  if (fs.existsSync(path.join(root, file))) {
    issue(file, "Forbidden authorization/signed-resolution/decision file exists. This milestone must not record authorization or sign a resolution.");
  }
}
...
if (fs.existsSync(configPath)) {
  ...
  if (config.resolutionSigningAuthorizationPackagePrepared !== true) {
    issue("resolutionSigningAuthorizationPackagePrepared", "Resolution signing authorization package should be prepared.");
  }
  ...
  const mustRemainFalse = [
    "resolutionSigningAuthorizationRequested",
  ...
  for (const key of mustRemainFalse) {
    if (config[key] !== false) {
      issue(key, "Resolution authorization item must remain false.");
    }
  }
  ...
  if (record.resolutionHash) {
    issue("draftAuthorizationRecord.resolutionHash", "Resolution hash must remain empty.");
  }

  for (const [key, value] of Object.entries(config.requiredBeforeResolutionSigningAuthorization || {})) {
    if ([
      ...
      if (value !== true) {
        issue(`requiredBeforeResolutionSigningAuthorization.${key}`, "Preparation prerequisite should be true.");
      }
    } else if (value !== false) {
      issue(`requiredBeforeResolutionSigningAuthorization.${key}`, "Resolution authorization prerequisite must remain false until completed.");
    }
  }
...
if (fs.existsSync(resolutionPath)) {
  ...
  if (resolution.fullLaunchApproved !== false) {
    issue("resolution.fullLaunchApproved", "Full launch must remain not approved.");
  }
}
...
const result = {
  schema: "astra-full-launch-governance-resolution-authorization-validation-v0.1",
