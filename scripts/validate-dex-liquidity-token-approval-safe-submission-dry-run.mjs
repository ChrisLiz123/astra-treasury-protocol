...
const requiredFiles = [
  "configs/dex-liquidity-token-approval-safe-submission-dry-run.config.json",
  "docs/dex-liquidity-token-approval-safe-submission-dry-run/DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_SUBMISSION_DRY_RUN.md",
  "docs/dex-liquidity-token-approval-safe-submission-dry-run/DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_SUBMISSION_DRY_RUN_CHECKLIST.md",
  "docs/dex-liquidity-token-approval-safe-submission-dry-run/DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_SUBMISSION_DRY_RUN_BOUNDARIES.md",
  "docs/dex-liquidity-token-approval-safe-submission-dry-run/DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_SUBMISSION_DRY_RUN_RUNBOOK.md",
  "scripts/dry-run-dex-liquidity-token-approval-safe-submission.mjs",
  recordRelativePath,
...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required token approval Safe submission dry-run file.");
  }
}
...
if (issues.length === 0) {
  const config = readJson("configs/dex-liquidity-token-approval-safe-submission-dry-run.config.json");
  ...
  if (config.tokenApprovalSafeSubmissionDryRunComplete !== true) {
    issue("config.tokenApprovalSafeSubmissionDryRunComplete", "Config must show dry run complete.");
  }

  if (record.status !== "DEX_LIQUIDITY_TOKEN_APPROVAL_SAFE_SUBMISSION_DRY_RUN_COMPLETE_NOT_SUBMITTED_NO_APPROVALS_EXECUTED_NO_LIQUIDITY") {
    issue("record.status", `Unexpected dry-run status: ${record.status}`);
  }

  if (record.tokenApprovalSafeSubmissionDryRunComplete !== true || record.operatorSubmissionCommandReviewed !== true) {
    issue("record.dryRunFlags", "Dry run and operator command review must be complete.");
  }
  ...
  if (record.payloadHash !== payload.payloadHash) {
    issue("record.payloadHash", "Dry-run payload hash must match generated payload.");
  }

  if (!Array.isArray(record.dryRunTransactionChecks) || record.dryRunTransactionChecks.length !== payload.transactions.length) {
    issue("record.dryRunTransactionChecks", "Dry-run checks must match payload transaction count.");
  }
  ...
  for (const check of record.dryRunTransactionChecks || []) {
    for (const key of [
      ...
      if (check[key] !== true) {
        issue(`record.dryRunTransactionChecks.${check.id}.${key}`, `${key} must be true.`);
      }
    }
...
const result = {
  schema: "astra-dex-liquidity-token-approval-safe-submission-dry-run-validation-v0.1",
