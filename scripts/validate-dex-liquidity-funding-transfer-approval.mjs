...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required funding transfer approval file.");
  }
}
...
if (issues.length === 0) {
  ...
  if (config.approvalPrepared !== true || config.approvalOnly !== true) {
    issue("config", "Funding transfer approval must be prepared and approval-only.");
  }

  if (config.fundingTransferApprovalRecorded !== approvalRecordPresent) {
    issue("config.fundingTransferApprovalRecorded", "Config approval flag must match record presence.");
  }

  if (config.fundingTransferApproved !== approvalRecordPresent) {
    issue("config.fundingTransferApproved", "Config approved flag must be true only after record exists.");
  }

  for (const key of [
    "fundingTransferPayloadGenerationApproved",
  ...
  if (requirementsStatus.summary?.sourceAddressProvided !== true || requirementsStatus.summary?.sourceAddressRequiredBeforePayloadGeneration !== false) {
    issue("requirementsStatus.summary.sourceAddress", "Funding source address must be provided before transfer approval.");
  }
  ...
  if (requirementsStatus.summary?.fundingTransferPayloadGenerated !== false || requirementsStatus.summary?.fundingTransferExecuted !== false || requirementsStatus.summary?.treasuryFundsMoved !== false) {
    issue("requirementsStatus.summary.flags", "Requirements review must show no payload, no execution, and no funds moved.");
  }
  ...
  if (approvalRecordPresent) {
    ...
    if (record.schema !== "astra-dex-liquidity-funding-transfer-approval-record-v0.1") {
      issue("record.schema", "Invalid funding transfer approval record schema.");
    }

    if (record.status !== "DEX_LIQUIDITY_TREASURY_FUNDING_TRANSFER_APPROVED_NO_PAYLOAD_NO_FUNDS_MOVED") {
      issue("record.status", "Unexpected funding transfer approval status.");
    }

    if (record.fundingTransferApproved !== true) {
      issue("record.fundingTransferApproved", "Record must approve funding transfer plan.");
    }
...
const result = {
  schema: "astra-dex-liquidity-funding-transfer-approval-validation-v0.1",
