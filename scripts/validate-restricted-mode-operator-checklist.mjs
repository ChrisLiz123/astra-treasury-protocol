...
const requiredFiles = [
  "configs/restricted-mode-operator-checklist.config.json",
...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required restricted-mode operator checklist file.");
  }
}
...
if (issues.length === 0) {
  ...
  if (config.operatorChecklistPrepared !== true) {
    issue("operatorChecklistPrepared", "Operator checklist config must be prepared.");
  }
...
const result = {
  schema: "astra-restricted-mode-operator-checklist-validation-v0.1",
