...
const requiredFiles = [
  "configs/restricted-mode-operations-handoff.config.json",
...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required restricted-mode operations handoff file.");
  }
}
...
if (issues.length === 0) {
  ...
  if (config.handoffPrepared !== true) {
    issue("handoffPrepared", "Operations handoff config must be prepared.");
  }
...
const result = {
  schema: "astra-restricted-mode-operations-handoff-validation-v0.1",
