...
PKG_NAME="astra-treasury-protocol-v0.1.1-mainnet-runbook"
...
cp -a docs/ops "$STAGE/docs-ops" 2>/dev/null || true
cp -a docs/security "$STAGE/docs-security" 2>/dev/null || true
cp -a configs "$STAGE/configs"
cp -a scripts/validate-mainnet-runbook.sh "$STAGE/"
