...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-safe-payload"
...
cp -a configs/dex-liquidity-token-approval-safe-payload.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-safe-payload "$STAGE/docs/dex-liquidity-token-approval-safe-payload"
cp -a services/dex-liquidity-token-approval-safe-payload "$STAGE/services-dex-liquidity-token-approval-safe-payload"
cp -a scripts/generate-dex-liquidity-token-approval-safe-payload.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-safe-payload.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-payload.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-payload-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-token-approval-safe-payload/dex-liquidity-token-approval-safe-payload-generation.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-token-approval-safe-payload/dex-liquidity-token-approval-safe-payload-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a public-docs/dex-liquidity-token-approval-payload-generation-approval-status.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-token-approval-payload-generation-approval/dex-liquidity-token-approval-payload-generation-approval-record.json "$STAGE/evidence/"
cp -a public-docs/dex-liquidity-token-approval-requirements-recheck-status.json "$STAGE/evidence/"
...
if find "$STAGE" -type f | grep -E "(token-approval-executed|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX token approval Safe payload package contains executed/liquidity/public/private artifacts."
  exit 1
fi
