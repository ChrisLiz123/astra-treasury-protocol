...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-payload-generation-approval"
...
cp -a configs/dex-liquidity-token-approval-payload-generation-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-payload-generation-approval "$STAGE/docs/dex-liquidity-token-approval-payload-generation-approval"
cp -a services/dex-liquidity-token-approval-payload-generation-approval "$STAGE/services-dex-liquidity-token-approval-payload-generation-approval"
cp -a scripts/record-dex-liquidity-token-approval-payload-generation-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-payload-generation-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-payload-generation-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-payload-generation-approval-status.json "$STAGE/"

cp -a reports/dex-liquidity-token-approval-payload-generation-approval/dex-liquidity-token-approval-payload-generation-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-token-approval-payload-generation-approval/dex-liquidity-token-approval-payload-generation-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(token-approval-safe-payload|token-approval-executed|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX token approval payload generation approval package contains token-approval/liquidity/public/private artifacts."
  exit 1
fi
