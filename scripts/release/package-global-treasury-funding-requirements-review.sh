...
PKG_NAME="astra-treasury-protocol-v0.1.1-global-treasury-funding-requirements-review"
STAGE="release/staging/$PKG_NAME"
ARCHIVE="release/$PKG_NAME.tar.gz"

rm -rf release/staging
rm -f "$ARCHIVE"
mkdir -p "$STAGE/evidence" "$STAGE/docs" "$STAGE/live"

cp -a configs/global-treasury-funding-requirements-review.config.json "$STAGE/"
cp -a docs/global-treasury-funding-requirements-review "$STAGE/docs/global-treasury-funding-requirements-review"
cp -a services/global-treasury-funding-requirements-review "$STAGE/services-global-treasury-funding-requirements-review"
cp -a scripts/review-global-treasury-funding-requirements.mjs "$STAGE/"
cp -a scripts/validate-global-treasury-funding-requirements-review.mjs "$STAGE/"
cp -a public-docs/global-treasury-funding-requirements-review.html "$STAGE/"
cp -a public-docs/global-treasury-funding-requirements-review-status.json "$STAGE/"

cp -a reports/global-treasury-funding-requirements-review/global-treasury-funding-requirements-review.json "$STAGE/evidence/"
cp -a reports/global-treasury-funding-requirements-review/global-treasury-funding-requirements-review-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/global-treasury-funding-approval/global-treasury-funding-approval-record.json "$STAGE/evidence/"
cp -a reports/global-treasury-funding/approval/global-treasury-funding-approved.json "$STAGE/evidence/"
...
cp -a public-docs/global-treasury-funding-approval-status.json "$STAGE/evidence/"
cp -a public-docs/global-treasury-funding-approved-status.json "$STAGE/evidence/"
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(global-treasury-funding-safe-payload\.json|global-treasury-funding-payload-status\.json|global-treasury-funding-live-status\.json|treasury-funding-executed|global-treasury-funding-executed|funds-moved|safe-submission|safe-execution)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: Global treasury funding requirements review package contains private/funding payload/submission/execution/fund movement files."
  exit 1
fi
