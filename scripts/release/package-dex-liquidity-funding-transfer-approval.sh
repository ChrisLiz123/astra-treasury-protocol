...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-funding-transfer-approval"
...
cp -a configs/dex-liquidity-funding-transfer-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-funding-transfer-approval "$STAGE/docs/dex-liquidity-funding-transfer-approval"
cp -a services/dex-liquidity-funding-transfer-approval "$STAGE/services-dex-liquidity-funding-transfer-approval"
cp -a scripts/record-dex-liquidity-funding-transfer-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-funding-transfer-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-approval-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-funding-transfer-approval/dex-liquidity-funding-transfer-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(funding-transfer-safe-payload|funds-moved|token-approval-safe-payload|liquidity-safe-payload|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX funding transfer approval package appears to contain funding/liquidity/public/private artifacts."
  exit 1
fi
