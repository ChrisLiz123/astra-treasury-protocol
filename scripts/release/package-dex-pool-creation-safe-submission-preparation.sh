...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-pool-creation-safe-submission-preparation"
...
cp -a configs/dex-pool-creation-safe-submission-preparation.config.json "$STAGE/"
cp -a docs/dex-pool-creation-safe-submission-preparation "$STAGE/docs/dex-pool-creation-safe-submission-preparation"
cp -a services/dex-pool-creation-safe-submission-preparation "$STAGE/services-dex-pool-creation-safe-submission-preparation"
cp -a scripts/prepare-dex-pool-creation-safe-submission.mjs "$STAGE/"
cp -a scripts/validate-dex-pool-creation-safe-submission-preparation.mjs "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-preparation.html "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-preparation-status.json "$STAGE/"
...
cp -a reports/dex-pool-creation-safe-submission-preparation/dex-pool-creation-safe-submission-preparation-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(safe-submission-record|safe-queued-record|safe-execution-record|pool-created|direct-execution-submitted|executed|private|secret|keystore)"; then
  echo "STOP: Safe submission preparation package appears to contain execution/private artifacts."
  exit 1
fi
