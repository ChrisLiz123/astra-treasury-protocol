...
PKG_NAME="astra-treasury-protocol-v0.1.1-capability-roadmap-governance"
...
cp -a configs/capability-roadmap.config.json "$STAGE/"
cp -a docs/roadmap "$STAGE/docs-roadmap"
cp -a services/capability-roadmap "$STAGE/services-capability-roadmap"
cp -a scripts/validate-capability-roadmap.mjs "$STAGE/"
cp -a public-docs/roadmap.html "$STAGE/"
cp -a public-docs/capability-roadmap-status.json "$STAGE/"
