...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-mint-parameter-review"
...
cp -a configs/dex-liquidity-mint-parameter-review.config.json "$STAGE/"
cp -a docs/dex-liquidity-mint-parameter-review "$STAGE/docs/dex-liquidity-mint-parameter-review"
cp -a services/dex-liquidity-mint-parameter-review "$STAGE/services-dex-liquidity-mint-parameter-review"
cp -a scripts/review-dex-liquidity-mint-parameters.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-mint-parameter-review.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-mint-parameter-review.html "$STAGE/"
cp -a public-docs/dex-liquidity-mint-parameter-review-status.json "$STAGE/"

cp -a reports/dex-liquidity-mint-parameter-review/dex-liquidity-mint-parameter-review.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-mint-parameter-review/dex-liquidity-mint-parameter-review-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(token-approval-safe-payload|liquidity-safe-payload|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: mint parameter review package appears to contain approval/liquidity/public/private artifacts."
  exit 1
fi
