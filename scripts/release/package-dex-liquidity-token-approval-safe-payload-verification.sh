...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-safe-payload-verification"
...
cp -a configs/dex-liquidity-token-approval-safe-payload-verification.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-safe-payload-verification "$STAGE/docs/dex-liquidity-token-approval-safe-payload-verification"
cp -a services/dex-liquidity-token-approval-safe-payload-verification "$STAGE/services-dex-liquidity-token-approval-safe-payload-verification"
cp -a scripts/verify-dex-liquidity-token-approval-safe-payload.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-safe-payload-verification.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-payload-verification.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-payload-verification-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-token-approval-safe-payload-verification/dex-liquidity-token-approval-safe-payload-verification-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(token-approval-executed|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX token approval verification package contains executed/liquidity/public/private artifacts."
  exit 1
fi
