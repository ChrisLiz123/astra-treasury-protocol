...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-safe-pending-signatures"
...
cp -a configs/dex-liquidity-token-approval-safe-pending-signatures.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-safe-pending-signatures "$STAGE/docs/dex-liquidity-token-approval-safe-pending-signatures"
cp -a services/dex-liquidity-token-approval-safe-pending-signatures "$STAGE/services-dex-liquidity-token-approval-safe-pending-signatures"
cp -a scripts/monitor-dex-liquidity-token-approval-safe-pending-signatures.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-safe-pending-signatures.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-pending-signatures.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-pending-signatures-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-token-approval-safe-pending-signatures/dex-liquidity-token-approval-safe-pending-signatures-status.json "$STAGE/evidence/" 2>/dev/null || true
...
cp -a public-docs/dex-liquidity-token-approval-safe-submission-dry-run-status.json "$STAGE/evidence/"
...
if find "$STAGE" -type f | grep -E "(token-approval-executed|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX token approval Safe pending signatures package contains executed/liquidity/public/private artifacts."
  exit 1
fi
