...
PKG_NAME="astra-treasury-protocol-v0.1.1-token-wallet-readiness"
...
cp -a services/wallet "$STAGE/services-wallet"
cp -a public-docs/wallet.html "$STAGE/"
cp -a public-docs/wallet-status.json "$STAGE/"
cp -a docs/token/WALLET_VISIBILITY_READINESS.md "$STAGE/"
cp -a docs/token/MANUAL_WALLET_ADD_INSTRUCTIONS.md "$STAGE/"
cp -a docs/token/TOKEN_PROVIDER_SUBMISSION_READINESS.md "$STAGE/"
cp -a docs/token/ASTRATOKEN_IDENTITY_CHECKLIST.md "$STAGE/"
