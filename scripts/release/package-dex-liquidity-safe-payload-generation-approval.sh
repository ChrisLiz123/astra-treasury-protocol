...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-safe-payload-generation-approval"
...
cp -a configs/dex-liquidity-safe-payload-generation-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-safe-payload-generation-approval "$STAGE/docs/dex-liquidity-safe-payload-generation-approval"
cp -a services/dex-liquidity-safe-payload-generation-approval "$STAGE/services-dex-liquidity-safe-payload-generation-approval"
cp -a scripts/record-dex-liquidity-safe-payload-generation-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-safe-payload-generation-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-safe-payload-generation-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-safe-payload-generation-approval-status.json "$STAGE/"

cp -a reports/dex-liquidity-safe-payload-generation-approval/dex-liquidity-safe-payload-generation-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-safe-payload-generation-approval/dex-liquidity-safe-payload-generation-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(liquidity-safe-payload\.json|dex-liquidity-safe-payload-status\.json|liquidity-added\.json|position-minted\.json|dex-liquidity-added-status\.json|dex-public-trading-live-status\.json)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity Safe payload generation approval package contains private/Safe-payload/liquidity/public-trading files."
  exit 1
fi
