...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-buy-page-activation-live"
...
cp -a configs/dex-buy-page-activation-live.config.json "$STAGE/"
cp -a docs/dex-buy-page-activation-live "$STAGE/docs/dex-buy-page-activation-live"
cp -a services/dex-buy-page-activation-live "$STAGE/services-dex-buy-page-activation-live"
cp -a scripts/record-dex-buy-page-activation-live.mjs "$STAGE/"
cp -a scripts/validate-dex-buy-page-activation-live.mjs "$STAGE/"
cp -a public-docs/dex-buy-page-activation-live.html "$STAGE/"
cp -a public-docs/dex-buy-page-activation-live-status.json "$STAGE/"
cp -a public-docs/buy.html "$STAGE/"
cp -a public-docs/dex-public-trading-live-status.json "$STAGE/"
cp -a public-docs/dex-buy-page-activated-status.json "$STAGE/"

cp -a reports/dex-buy-page-activation-live/dex-buy-page-activation-live-record.json "$STAGE/evidence/"
cp -a reports/dex-buy-page-activation-live/dex-buy-page-activation-live-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX buy page activation live package contains private/full-launch files."
  exit 1
fi
