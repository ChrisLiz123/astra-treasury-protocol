...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-funding-transfer-payload-generation-approval"
...
cp -a configs/dex-liquidity-funding-transfer-payload-generation-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-funding-transfer-payload-generation-approval "$STAGE/docs/dex-liquidity-funding-transfer-payload-generation-approval"
cp -a services/dex-liquidity-funding-transfer-payload-generation-approval "$STAGE/services-dex-liquidity-funding-transfer-payload-generation-approval"
cp -a scripts/record-dex-liquidity-funding-transfer-payload-generation-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-funding-transfer-payload-generation-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-payload-generation-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-payload-generation-approval-status.json "$STAGE/"

cp -a reports/dex-liquidity-funding-transfer-payload-generation-approval/dex-liquidity-funding-transfer-payload-generation-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-funding-transfer-payload-generation-approval/dex-liquidity-funding-transfer-payload-generation-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a public-docs/dex-liquidity-funding-transfer-approval-status.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-funding-transfer-approval/dex-liquidity-funding-transfer-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a public-docs/dex-liquidity-funding-transfer-requirements-status.json "$STAGE/evidence/"
...
if find "$STAGE" -type f | grep -E "(funding-transfer-safe-payload|funds-moved|token-approval-safe-payload|liquidity-safe-payload|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX funding-transfer payload generation approval package appears to contain funding/liquidity/public/private artifacts."
  exit 1
fi
