...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-public-trading-link-approval"
...
cp -a configs/dex-public-trading-link-approval.config.json "$STAGE/"
cp -a docs/dex-public-trading-link-approval "$STAGE/docs/dex-public-trading-link-approval"
cp -a services/dex-public-trading-link-approval "$STAGE/services-dex-public-trading-link-approval"
cp -a scripts/record-dex-public-trading-link-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-public-trading-link-approval.mjs "$STAGE/"
cp -a public-docs/dex-public-trading-link-approval.html "$STAGE/"
cp -a public-docs/dex-public-trading-link-approval-status.json "$STAGE/"

cp -a reports/dex-public-trading-link-approval/dex-public-trading-link-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-public-trading-link-approval/dex-public-trading-link-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-public-trading-approval/dex-public-trading-approval-record.json "$STAGE/evidence/"
cp -a reports/dex-public-trading-readiness-review/dex-public-trading-readiness-review.json "$STAGE/evidence/"
...
cp -a public-docs/dex-public-trading-readiness-review-status.json "$STAGE/evidence/"
...
if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX public trading link approval package contains private/public-trading-live/buy-page/full-launch files."
  exit 1
fi
