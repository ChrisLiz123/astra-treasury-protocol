...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-mint-calldata-generation-approval"
...
cp -a configs/dex-liquidity-mint-calldata-generation-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-mint-calldata-generation-approval "$STAGE/docs/dex-liquidity-mint-calldata-generation-approval"
cp -a services/dex-liquidity-mint-calldata-generation-approval "$STAGE/services-dex-liquidity-mint-calldata-generation-approval"
cp -a scripts/record-dex-liquidity-mint-calldata-generation-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-mint-calldata-generation-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-mint-calldata-generation-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-mint-calldata-generation-approval-status.json "$STAGE/"

cp -a reports/dex-liquidity-mint-calldata-generation-approval/dex-liquidity-mint-calldata-generation-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-mint-calldata-generation-approval/dex-liquidity-mint-calldata-generation-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(liquidity-mint-calldata\.json|dex-liquidity-mint-calldata-status\.json|liquidity-safe-payload\.json|liquidity-added\.json|position-minted\.json|dex-liquidity-added-status\.json|dex-public-trading-live-status\.json)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity mint calldata approval package contains private/calldata/liquidity/public-trading files."
  exit 1
fi
