...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-pool-creation-post-execution-verification"
...
cp -a configs/dex-pool-creation-post-execution-verification.config.json "$STAGE/"
cp -a docs/dex-pool-creation-post-execution-verification "$STAGE/docs/dex-pool-creation-post-execution-verification"
cp -a services/dex-pool-creation-post-execution-verification "$STAGE/services-dex-pool-creation-post-execution-verification"
cp -a scripts/verify-dex-pool-creation-post-execution.mjs "$STAGE/"
cp -a scripts/validate-dex-pool-creation-post-execution-verification.mjs "$STAGE/"
cp -a public-docs/dex-pool-creation-post-execution-verification.html "$STAGE/"
cp -a public-docs/dex-pool-creation-post-execution-verification-status.json "$STAGE/"

cp -a reports/dex-pool-creation-post-execution-verification/dex-pool-creation-post-execution-verification.json "$STAGE/evidence/"
cp -a reports/dex-pool-creation-post-execution-verification/dex-pool-creation-post-execution-verification-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(private|secret|keystore)"; then
  echo "STOP: post-execution verification package appears to contain private artifacts."
  exit 1
fi
