...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-safe-submission-preparation"
...
cp -a configs/dex-liquidity-token-approval-safe-submission-preparation.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-safe-submission-preparation "$STAGE/docs/dex-liquidity-token-approval-safe-submission-preparation"
cp -a services/dex-liquidity-token-approval-safe-submission-preparation "$STAGE/services-dex-liquidity-token-approval-safe-submission-preparation"
cp -a scripts/prepare-dex-liquidity-token-approval-safe-submission.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-safe-submission-preparation.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-submission-preparation.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-submission-preparation-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-token-approval-safe-submission-preparation/dex-liquidity-token-approval-safe-submission-preparation-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(safe-submission-live|token-approval-executed|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX token approval Safe submission preparation package contains live/executed/liquidity/public/private artifacts."
  exit 1
fi
