...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-parameter-selection-import"
STAGE="release/staging/$PKG_NAME"
ARCHIVE="release/$PKG_NAME.tar.gz"

rm -rf release/staging
rm -f "$ARCHIVE"
mkdir -p "$STAGE/evidence" "$STAGE/docs" "$STAGE/import"

cp -a configs/dex-liquidity-parameter-selection-import.config.json "$STAGE/"
cp -a docs/dex-liquidity-parameter-selection "$STAGE/docs/dex-liquidity-parameter-selection"
cp -a services/dex-liquidity-parameter-selection "$STAGE/services-dex-liquidity-parameter-selection"
cp -a scripts/import-dex-liquidity-parameter-selection.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-parameter-selection.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-parameter-selection.html "$STAGE/"
cp -a public-docs/dex-liquidity-parameter-selection-status.json "$STAGE/"

cp -a reports/dex-liquidity-parameter-selection/import/dex-liquidity-parameter-selection.json "$STAGE/import/" 2>/dev/null || true
