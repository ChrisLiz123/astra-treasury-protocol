...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-pool-creation-safe-submission-dry-run-review"
...
cp -a configs/dex-pool-creation-safe-submission-dry-run.config.json "$STAGE/"
cp -a docs/dex-pool-creation-safe-submission-dry-run "$STAGE/docs/dex-pool-creation-safe-submission-dry-run"
cp -a services/dex-pool-creation-safe-submission-dry-run "$STAGE/services-dex-pool-creation-safe-submission-dry-run"
cp -a scripts/prepare-dex-pool-creation-safe-submission-dry-run.mjs "$STAGE/"
cp -a scripts/validate-dex-pool-creation-safe-submission-dry-run.mjs "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-dry-run.html "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-dry-run-status.json "$STAGE/"

cp -a reports/dex-pool-creation-safe-submission-dry-run/dex-pool-creation-safe-submission-dry-run-review.json "$STAGE/evidence/"
cp -a reports/dex-pool-creation-safe-submission-dry-run/dex-pool-creation-safe-submission-dry-run-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-pool-creation-safe-submission-preparation/dex-pool-creation-safe-submission-preparation.json "$STAGE/evidence/"
...
if find "$STAGE" -type f | grep -E "(safe-submission-record|safe-queued-record|safe-execution-record|pool-created|direct-execution-submitted|executed|private|secret|keystore)"; then
  echo "STOP: Safe submission dry run package appears to contain execution/private artifacts."
  exit 1
fi
