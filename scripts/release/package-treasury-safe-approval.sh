...
PKG_NAME="astra-treasury-protocol-v0.1.1-treasury-safe-approval-package"
...
cp -a configs/treasury-safe-approval.config.json "$STAGE/"
cp -a docs/treasury-safe "$STAGE/docs-treasury-safe"
cp -a services/treasury-safe "$STAGE/services-treasury-safe"
cp -a scripts/validate-treasury-safe-approval.mjs "$STAGE/"
cp -a public-docs/treasury-safe.html "$STAGE/"
cp -a public-docs/treasury-safe-status.json "$STAGE/"
