...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-safe-submission-live"
...
cp -a configs/dex-liquidity-safe-submission-live.config.json "$STAGE/"
cp -a docs/dex-liquidity-safe-submission-live "$STAGE/docs/dex-liquidity-safe-submission-live"
cp -a services/dex-liquidity-safe-submission-live "$STAGE/services-dex-liquidity-safe-submission-live"
cp -a scripts/record-dex-liquidity-safe-submission-live.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-safe-submission-live.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-safe-submission-live.html "$STAGE/"
cp -a public-docs/dex-liquidity-safe-submission-live-status.json "$STAGE/"

cp -a reports/dex-liquidity-safe-submission-live/dex-liquidity-safe-submission-live-record.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-safe-submission-live/dex-liquidity-safe-submission-live-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-safe-submission-dry-run/dex-liquidity-safe-submission-dry-run.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-safe-submission-preparation/dex-liquidity-safe-submission-preparation.json "$STAGE/evidence/"
...
cp -a public-docs/dex-liquidity-safe-submission-dry-run-status.json "$STAGE/evidence/"
cp -a public-docs/dex-liquidity-safe-submission-preparation-status.json "$STAGE/evidence/"
cp -a public-docs/dex-liquidity-safe-submission-approval-status.json "$STAGE/evidence/"
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(liquidity-added\.json|position-minted\.json|dex-liquidity-added-status\.json|dex-public-trading-live-status\.json)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity Safe submission live package contains private/executed-liquidity/public-trading files."
  exit 1
fi
