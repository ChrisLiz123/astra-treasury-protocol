...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-pool-creation-safe-submission-live"
...
cp -a configs/dex-pool-creation-safe-submission-live.config.json "$STAGE/"
cp -a docs/dex-pool-creation-safe-submission-live "$STAGE/docs/dex-pool-creation-safe-submission-live"
cp -a services/dex-pool-creation-safe-submission-live "$STAGE/services-dex-pool-creation-safe-submission-live"
cp -a scripts/record-dex-pool-creation-safe-submission-live.mjs "$STAGE/"
cp -a scripts/validate-dex-pool-creation-safe-submission-live.mjs "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-live.html "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-live-status.json "$STAGE/"
...
cp -a reports/dex-pool-creation-safe-submission-live/dex-pool-creation-safe-submission-live-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-pool-creation-safe-payload-generation/generated/dex-pool-creation-safe-payload.json "$STAGE/generated/"
cp -a public-docs/dex-pool-creation-safe-submission-execution-approval-status.json "$STAGE/evidence/"
...
if find "$STAGE" -type f | grep -E "(safe-execution-record|pool-created|direct-execution-submitted|executed|signature|private|secret|keystore)"; then
  echo "STOP: Safe submission live package appears to contain execution/private/signature artifacts."
  exit 1
fi
