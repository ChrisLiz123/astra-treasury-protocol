...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-pool-creation-safe-payload-verification-review"
...
cp -a configs/dex-pool-creation-safe-payload-verification.config.json "$STAGE/"
cp -a docs/dex-pool-creation-safe-payload-verification "$STAGE/docs/dex-pool-creation-safe-payload-verification"
cp -a services/dex-pool-creation-safe-payload-verification "$STAGE/services-dex-pool-creation-safe-payload-verification"
cp -a scripts/verify-dex-pool-creation-safe-payload.mjs "$STAGE/"
cp -a scripts/validate-dex-pool-creation-safe-payload-verification.mjs "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-payload-verification.html "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-payload-verification-status.json "$STAGE/"

cp -a reports/dex-pool-creation-safe-payload-generation/generated/dex-pool-creation-safe-payload.json "$STAGE/generated/"
cp -a reports/dex-pool-creation-safe-payload-verification/dex-pool-creation-safe-payload-verification-review.json "$STAGE/evidence/"
cp -a reports/dex-pool-creation-safe-payload-verification/dex-pool-creation-safe-payload-verification-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(pool-created|direct-execution-submitted|executed|signature|private|secret|keystore)"; then
  echo "STOP: verification package appears to contain execution/private artifacts."
  exit 1
fi
