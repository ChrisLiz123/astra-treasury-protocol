...
PKG_NAME="astra-treasury-protocol-v0.1.1-full-launch-approval"
...
cp -a configs/full-launch-approval.config.json "$STAGE/"
cp -a docs/full-launch-approval "$STAGE/docs/full-launch-approval"
cp -a services/full-launch-approval "$STAGE/services-full-launch-approval"
cp -a scripts/record-full-launch-approval.mjs "$STAGE/"
cp -a scripts/validate-full-launch-approval.mjs "$STAGE/"
cp -a public-docs/full-launch-approval.html "$STAGE/"
cp -a public-docs/full-launch-approval-status.json "$STAGE/"
cp -a public-docs/full-launch-approved-status.json "$STAGE/" 2>/dev/null || true

cp -a reports/full-launch-approval/full-launch-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/full-launch-approval/full-launch-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/full-launch/approval/full-launch-approved.json "$STAGE/evidence/" 2>/dev/null || true
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(full-launch-live-status\.json|full-launch-live|treasury-funding-executed|global-treasury-funding-executed)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: Full launch approval package contains private/full-launch-live/treasury-execution files."
  exit 1
fi
