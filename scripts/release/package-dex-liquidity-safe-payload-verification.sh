...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-safe-payload-verification"
...
cp -a configs/dex-liquidity-safe-payload-verification.config.json "$STAGE/"
cp -a docs/dex-liquidity-safe-payload-verification "$STAGE/docs/dex-liquidity-safe-payload-verification"
cp -a services/dex-liquidity-safe-payload-verification "$STAGE/services-dex-liquidity-safe-payload-verification"
cp -a scripts/verify-dex-liquidity-safe-payload.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-safe-payload-verification.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-safe-payload-verification.html "$STAGE/"
cp -a public-docs/dex-liquidity-safe-payload-verification-status.json "$STAGE/"

cp -a reports/dex-liquidity-safe-payload-verification/dex-liquidity-safe-payload-verification.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-safe-payload-verification/dex-liquidity-safe-payload-verification-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity Safe payload verification package contains private/submitted/liquidity/public-trading files."
  exit 1
fi
