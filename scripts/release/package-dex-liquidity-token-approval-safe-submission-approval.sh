...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-safe-submission-approval"
...
cp -a configs/dex-liquidity-token-approval-safe-submission-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-safe-submission-approval "$STAGE/docs/dex-liquidity-token-approval-safe-submission-approval"
cp -a services/dex-liquidity-token-approval-safe-submission-approval "$STAGE/services-dex-liquidity-token-approval-safe-submission-approval"
cp -a scripts/record-dex-liquidity-token-approval-safe-submission-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-safe-submission-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-submission-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-submission-approval-status.json "$STAGE/"

cp -a reports/dex-liquidity-token-approval-safe-submission-approval/dex-liquidity-token-approval-safe-submission-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-token-approval-safe-submission-approval/dex-liquidity-token-approval-safe-submission-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(safe-submission-live|token-approval-executed|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX token approval Safe submission approval package contains live/executed/liquidity/public/private artifacts."
  exit 1
fi
