...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-parameter-review"
...
cp -a configs/dex-liquidity-parameter-review.config.json "$STAGE/"
cp -a docs/dex-liquidity-parameters "$STAGE/docs/dex-liquidity-parameters"
cp -a services/dex-liquidity-parameters "$STAGE/services-dex-liquidity-parameters"
cp -a scripts/validate-dex-liquidity-parameter-review.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-parameters.html "$STAGE/"
cp -a public-docs/dex-liquidity-parameters-status.json "$STAGE/"

cp -a public-docs/dex-liquidity-path-status.json "$STAGE/evidence/"
cp -a public-docs/capability-request-review-status.json "$STAGE/evidence/"
