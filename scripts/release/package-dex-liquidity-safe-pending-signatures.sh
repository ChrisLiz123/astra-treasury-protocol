...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-safe-pending-signatures"
...
cp -a configs/dex-liquidity-safe-pending-signatures.config.json "$STAGE/"
cp -a docs/dex-liquidity-safe-pending-signatures "$STAGE/docs/dex-liquidity-safe-pending-signatures"
cp -a services/dex-liquidity-safe-pending-signatures "$STAGE/services-dex-liquidity-safe-pending-signatures"
cp -a scripts/monitor-dex-liquidity-safe-pending-signatures.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-safe-pending-signatures.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-safe-pending-signatures.html "$STAGE/"
cp -a public-docs/dex-liquidity-safe-pending-signatures-status.json "$STAGE/"

cp -a reports/dex-liquidity-safe-pending-signatures/dex-liquidity-safe-pending-signatures-monitoring.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-safe-pending-signatures/dex-liquidity-safe-pending-signatures-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity Safe pending signatures package contains private/executed-liquidity/public-trading files."
  exit 1
fi
