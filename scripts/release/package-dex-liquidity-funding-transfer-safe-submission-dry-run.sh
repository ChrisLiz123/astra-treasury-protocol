...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-funding-transfer-safe-submission-dry-run"
...
cp -a configs/dex-liquidity-funding-transfer-safe-submission-dry-run.config.json "$STAGE/"
cp -a docs/dex-liquidity-funding-transfer-safe-submission-dry-run "$STAGE/docs/dex-liquidity-funding-transfer-safe-submission-dry-run"
cp -a services/dex-liquidity-funding-transfer-safe-submission-dry-run "$STAGE/services-dex-liquidity-funding-transfer-safe-submission-dry-run"
cp -a scripts/dry-run-dex-liquidity-funding-transfer-safe-submission.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-funding-transfer-safe-submission-dry-run.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-safe-submission-dry-run.html "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-safe-submission-dry-run-status.json "$STAGE/"

cp -a reports/dex-liquidity-funding-transfer-safe-submission-dry-run/dex-liquidity-funding-transfer-safe-submission-dry-run.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-funding-transfer-safe-submission-dry-run/dex-liquidity-funding-transfer-safe-submission-dry-run-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(safe-submission-live|funds-moved|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX funding-transfer Safe submission dry-run package appears to contain live/liquidity/public/private artifacts."
  exit 1
fi
