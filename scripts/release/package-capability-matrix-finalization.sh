...
PKG_NAME="astra-treasury-protocol-v0.1.1-capability-matrix-finalization"
...
cp -a configs/capability-matrix.config.json "$STAGE/"
cp -a docs/capability-matrix "$STAGE/docs-capability-matrix"
cp -a services/capability-matrix "$STAGE/services-capability-matrix"
cp -a scripts/validate-capability-matrix.mjs "$STAGE/"
cp -a public-docs/capability-matrix.html "$STAGE/"
cp -a public-docs/capability-matrix-status.json "$STAGE/"
