...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-requirements"
...
cp -a configs/dex-liquidity-token-approval-requirements.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-requirements "$STAGE/docs/dex-liquidity-token-approval-requirements"
cp -a services/dex-liquidity-token-approval-requirements "$STAGE/services-dex-liquidity-token-approval-requirements"
cp -a scripts/review-dex-liquidity-token-approval-requirements.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-requirements.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-requirements.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-requirements-status.json "$STAGE/"

cp -a reports/dex-liquidity-token-approval-requirements/dex-liquidity-token-approval-requirements-review.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-token-approval-requirements/dex-liquidity-token-approval-requirements-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(token-approval-safe-payload|liquidity-safe-payload|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: token approval requirements package appears to contain approval/liquidity/public/private artifacts."
  exit 1
fi
