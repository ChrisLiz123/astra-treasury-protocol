...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-buy-page-activation-approval"
...
cp -a configs/dex-buy-page-activation-approval.config.json "$STAGE/"
cp -a docs/dex-buy-page-activation-approval "$STAGE/docs/dex-buy-page-activation-approval"
cp -a services/dex-buy-page-activation-approval "$STAGE/services-dex-buy-page-activation-approval"
cp -a scripts/record-dex-buy-page-activation-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-buy-page-activation-approval.mjs "$STAGE/"
cp -a public-docs/dex-buy-page-activation-approval.html "$STAGE/"
cp -a public-docs/dex-buy-page-activation-approval-status.json "$STAGE/"

cp -a reports/dex-buy-page-activation-approval/dex-buy-page-activation-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-buy-page-activation-approval/dex-buy-page-activation-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(dex-buy-page-activated-status\.json|dex-buy-page-activation-live-status\.json|full-launch-approved-status\.json|buy-page-activated|full-launch-approved)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX buy page activation approval package contains private/buy-page-live/full-launch files."
  exit 1
fi
