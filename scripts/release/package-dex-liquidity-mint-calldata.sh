...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-mint-calldata"
...
cp -a configs/dex-liquidity-mint-calldata.config.json "$STAGE/"
cp -a docs/dex-liquidity-mint-calldata "$STAGE/docs/dex-liquidity-mint-calldata"
cp -a services/dex-liquidity-mint-calldata "$STAGE/services-dex-liquidity-mint-calldata"
cp -a scripts/generate-dex-liquidity-mint-calldata.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-mint-calldata.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-mint-calldata.html "$STAGE/"
cp -a public-docs/dex-liquidity-mint-calldata-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-mint-calldata/dex-liquidity-mint-calldata-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity mint calldata package contains private/liquidity/public-trading files."
  exit 1
fi
