...
PKG_NAME="astra-treasury-protocol-v0.1.1-restricted-launch-announcement"
...
cp -a configs/restricted-launch-announcement.config.json "$STAGE/"
cp -a docs/announcement "$STAGE/docs-announcement"
cp -a services/announcement "$STAGE/services-announcement"
cp -a scripts/validate-restricted-launch-announcement.mjs "$STAGE/"
cp -a public-docs/announcement.html "$STAGE/"
cp -a public-docs/announcement-status.json "$STAGE/"
