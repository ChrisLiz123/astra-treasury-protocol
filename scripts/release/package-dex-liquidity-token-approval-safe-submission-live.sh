...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-token-approval-safe-submission-live"
...
cp -a configs/dex-liquidity-token-approval-safe-submission-live.config.json "$STAGE/"
cp -a docs/dex-liquidity-token-approval-safe-submission-live "$STAGE/docs/dex-liquidity-token-approval-safe-submission-live"
cp -a services/dex-liquidity-token-approval-safe-submission-live "$STAGE/services-dex-liquidity-token-approval-safe-submission-live"
cp -a scripts/record-dex-liquidity-token-approval-safe-submission-live.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-token-approval-safe-submission-live.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-submission-live.html "$STAGE/"
cp -a public-docs/dex-liquidity-token-approval-safe-submission-live-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-token-approval-safe-submission-live/dex-liquidity-token-approval-safe-submission-live-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(token-approval-executed|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX token approval Safe submission live package contains executed/liquidity/public/private artifacts."
  exit 1
fi
