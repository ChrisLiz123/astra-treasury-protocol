...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-safe-submission-preparation"
...
cp -a configs/dex-liquidity-safe-submission-preparation.config.json "$STAGE/"
cp -a docs/dex-liquidity-safe-submission-preparation "$STAGE/docs/dex-liquidity-safe-submission-preparation"
cp -a services/dex-liquidity-safe-submission-preparation "$STAGE/services-dex-liquidity-safe-submission-preparation"
cp -a scripts/prepare-dex-liquidity-safe-submission.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-safe-submission-preparation.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-safe-submission-preparation.html "$STAGE/"
cp -a public-docs/dex-liquidity-safe-submission-preparation-status.json "$STAGE/"

cp -a reports/dex-liquidity-safe-submission-preparation/dex-liquidity-safe-submission-preparation.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-safe-submission-preparation/dex-liquidity-safe-submission-preparation-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity Safe submission preparation package contains private/submitted/liquidity/public-trading files."
  exit 1
fi
