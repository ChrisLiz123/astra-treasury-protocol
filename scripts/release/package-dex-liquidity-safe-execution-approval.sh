...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-safe-execution-approval"
...
cp -a configs/dex-liquidity-safe-execution-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-safe-execution-approval "$STAGE/docs/dex-liquidity-safe-execution-approval"
cp -a services/dex-liquidity-safe-execution-approval "$STAGE/services-dex-liquidity-safe-execution-approval"
cp -a scripts/record-dex-liquidity-safe-execution-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-safe-execution-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-safe-execution-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-safe-execution-approval-status.json "$STAGE/"

cp -a reports/dex-liquidity-safe-execution-approval/dex-liquidity-safe-execution-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-safe-execution-approval/dex-liquidity-safe-execution-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity Safe execution approval package contains private/executed-liquidity/public-trading files."
  exit 1
fi
