...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-pool-creation-safe-submission-approval"
...
cp -a configs/dex-pool-creation-safe-submission-approval.config.json "$STAGE/"
cp -a docs/dex-pool-creation-safe-submission-approval "$STAGE/docs/dex-pool-creation-safe-submission-approval"
cp -a services/dex-pool-creation-safe-submission-approval "$STAGE/services-dex-pool-creation-safe-submission-approval"
cp -a scripts/record-dex-pool-creation-safe-submission-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-pool-creation-safe-submission-approval.mjs "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-approval.html "$STAGE/"
cp -a public-docs/dex-pool-creation-safe-submission-approval-status.json "$STAGE/"

cp -a reports/dex-pool-creation-safe-payload-generation/generated/dex-pool-creation-safe-payload.json "$STAGE/generated/"
cp -a reports/dex-pool-creation-safe-submission-approval/dex-pool-creation-safe-submission-approval-record.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-pool-creation-safe-submission-approval/dex-pool-creation-safe-submission-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(safe-submission-record|safe-queued-record|safe-execution-record|pool-created|direct-execution-submitted|executed|private|secret|keystore)"; then
  echo "STOP: Safe submission approval package appears to contain execution/private artifacts."
  exit 1
fi
