...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-safe-submission-dry-run"
...
cp -a configs/dex-liquidity-safe-submission-dry-run.config.json "$STAGE/"
cp -a docs/dex-liquidity-safe-submission-dry-run "$STAGE/docs/dex-liquidity-safe-submission-dry-run"
cp -a services/dex-liquidity-safe-submission-dry-run "$STAGE/services-dex-liquidity-safe-submission-dry-run"
cp -a scripts/dry-run-dex-liquidity-safe-submission.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-safe-submission-dry-run.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-safe-submission-dry-run.html "$STAGE/"
cp -a public-docs/dex-liquidity-safe-submission-dry-run-status.json "$STAGE/"

cp -a reports/dex-liquidity-safe-submission-dry-run/dex-liquidity-safe-submission-dry-run.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-safe-submission-dry-run/dex-liquidity-safe-submission-dry-run-status.json "$STAGE/evidence/" 2>/dev/null || true
cp -a reports/dex-liquidity-safe-submission-preparation/dex-liquidity-safe-submission-preparation.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-safe-submission-approval/dex-liquidity-safe-submission-approval-record.json "$STAGE/evidence/"
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(dex-liquidity-safe-submission-live-record\.json|dex-liquidity-safe-pending-signatures-monitoring\.json|liquidity-added\.json|position-minted\.json|dex-liquidity-added-status\.json|dex-public-trading-live-status\.json)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX liquidity Safe submission dry-run package contains private/submitted/liquidity/public-trading files."
  exit 1
fi
