...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-treasury-funding-approval"
...
cp -a configs/dex-liquidity-treasury-funding-approval.config.json "$STAGE/"
cp -a docs/dex-liquidity-treasury-funding-approval "$STAGE/docs/dex-liquidity-treasury-funding-approval"
cp -a services/dex-liquidity-treasury-funding-approval "$STAGE/services-dex-liquidity-treasury-funding-approval"
cp -a scripts/record-dex-liquidity-treasury-funding-approval.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-treasury-funding-approval.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-treasury-funding-approval.html "$STAGE/"
cp -a public-docs/dex-liquidity-treasury-funding-approval-status.json "$STAGE/"
...
cp -a reports/dex-liquidity-treasury-funding-approval/dex-liquidity-treasury-funding-approval-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(funding-transfer-safe-payload|funds-moved|token-approval-safe-payload|liquidity-safe-payload|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX liquidity treasury funding approval package appears to contain funding/liquidity/public/private artifacts."
  exit 1
fi
