...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-liquidity-funding-transfer-safe-submission-preparation"
...
cp -a configs/dex-liquidity-funding-transfer-safe-submission-preparation.config.json "$STAGE/"
cp -a docs/dex-liquidity-funding-transfer-safe-submission-preparation "$STAGE/docs/dex-liquidity-funding-transfer-safe-submission-preparation"
cp -a services/dex-liquidity-funding-transfer-safe-submission-preparation "$STAGE/services-dex-liquidity-funding-transfer-safe-submission-preparation"
cp -a scripts/prepare-dex-liquidity-funding-transfer-safe-submission.mjs "$STAGE/"
cp -a scripts/validate-dex-liquidity-funding-transfer-safe-submission-preparation.mjs "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-safe-submission-preparation.html "$STAGE/"
cp -a public-docs/dex-liquidity-funding-transfer-safe-submission-preparation-status.json "$STAGE/"

cp -a reports/dex-liquidity-funding-transfer-safe-submission-preparation/dex-liquidity-funding-transfer-safe-submission-preparation.json "$STAGE/evidence/"
cp -a reports/dex-liquidity-funding-transfer-safe-submission-preparation/dex-liquidity-funding-transfer-safe-submission-preparation-status.json "$STAGE/evidence/" 2>/dev/null || true
...
if find "$STAGE" -type f | grep -E "(safe-submission-live|funds-moved|liquidity-added|position-minted|public-trading-live|private|secret|keystore)"; then
  echo "STOP: DEX funding-transfer Safe submission preparation package appears to contain live/liquidity/public/private artifacts."
  exit 1
fi
