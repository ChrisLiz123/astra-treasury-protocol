...
PKG_NAME="astra-treasury-protocol-v0.1.1-post-announcement-monitoring"
...
cp -a configs/post-announcement-monitoring.config.json "$STAGE/"
cp -a services/post-announcement-monitor "$STAGE/services-post-announcement-monitor"
cp -a docs/announcement/POST_PUBLICATION_24H_MONITORING_WINDOW.md "$STAGE/"
cp -a docs/announcement/POST_PUBLICATION_STATUS.md "$STAGE/"
cp -a public-docs/post-announcement.html "$STAGE/"
cp -a public-docs/post-announcement-status.json "$STAGE/"
