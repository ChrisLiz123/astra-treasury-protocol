...
PKG_NAME="astra-treasury-protocol-v0.1.1-public-transparency-index"
...
cp -a public-docs/transparency.html "$STAGE/"
cp -a public-docs/transparency-status.json "$STAGE/"
cp -a services/docs/generate-public-transparency-docs.mjs "$STAGE/"
cp -a services/public-site/server.mjs "$STAGE/"
cp -a deployments/base-sepolia.public.json "$STAGE/" 2>/dev/null || true
