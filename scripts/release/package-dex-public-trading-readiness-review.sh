...
PKG_NAME="astra-treasury-protocol-v0.1.1-dex-public-trading-readiness-review"
...
cp -a configs/dex-public-trading-readiness-review.config.json "$STAGE/"
cp -a docs/dex-public-trading-readiness-review "$STAGE/docs/dex-public-trading-readiness-review"
cp -a services/dex-public-trading-readiness-review "$STAGE/services-dex-public-trading-readiness-review"
cp -a scripts/review-dex-public-trading-readiness.mjs "$STAGE/"
cp -a scripts/validate-dex-public-trading-readiness-review.mjs "$STAGE/"
cp -a public-docs/dex-public-trading-readiness-review.html "$STAGE/"
cp -a public-docs/dex-public-trading-readiness-review-status.json "$STAGE/"

cp -a reports/dex-public-trading-readiness-review/dex-public-trading-readiness-review.json "$STAGE/evidence/"
cp -a reports/dex-public-trading-readiness-review/dex-public-trading-readiness-review-status.json "$STAGE/evidence/" 2>/dev/null || true
...
FORBIDDEN_PATTERN="(^|/)(node_modules|reports|backups|artifacts|cache|release|\.git|\.hardhat|out|artifacts-local|\.runtime)(/|$)|(^|/)(\.env|.*\.env|.*\.pem|.*\.key|.*keystore.*|mainnet-production\.config\.json)$|(^|/)(dex-public-trading-approval-status\.json|dex-public-trading-live-status\.json|dex-buy-page-activated-status\.json|full-launch-approved-status\.json)$"

if find "$STAGE" -type f -print | sed "s#^$STAGE/##" | grep -E "$FORBIDDEN_PATTERN"; then
  echo "STOP: DEX public trading readiness review package contains private/public-trading-approval/live/full-launch files."
  exit 1
fi
