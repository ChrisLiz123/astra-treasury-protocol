...
const configPath = path.join(root, "configs", "full-launch-governance-vote-opening.config.json");

const requiredFiles = [
  "docs/full-launch-governance-vote-opening/GOVERNANCE_VOTE_OPENING_PACKAGE.md",
  "docs/full-launch-governance-vote-opening/GOVERNANCE_VOTE_OPENING_CHECKLIST.md",
  "docs/full-launch-governance-vote-opening/GOVERNANCE_VOTE_NOTICE_TEMPLATE.md",
  "docs/full-launch-governance-vote-opening/GOVERNANCE_VOTE_EVIDENCE_PLAN.md",
  "docs/full-launch-governance-vote-opening/GOVERNANCE_VOTE_OPENING_DECISION_RECORD.md",
  "docs/full-launch-governance-vote-opening/GOVERNANCE_VOTE_OPENING_BLOCKERS.md",
...
if (!fs.existsSync(configPath)) {
  issue("configs/full-launch-governance-vote-opening.config.json", "Missing governance vote opening config.");
}

for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required governance vote opening file.");
  }
}
...
if (fs.existsSync(configPath)) {
  ...
  if (config.governanceVoteOpeningPackagePrepared !== true) {
    issue("governanceVoteOpeningPackagePrepared", "Governance vote opening package should be prepared.");
  }

  if (config.publicVoteNoticeDrafted !== true) {
    issue("publicVoteNoticeDrafted", "Public vote notice draft should be prepared.");
  }

  if (config.voteEvidencePlanPrepared !== true) {
    issue("voteEvidencePlanPrepared", "Vote evidence plan should be prepared.");
  }

  if (config.publicStatusUpdatePrepared !== true) {
    issue("publicStatusUpdatePrepared", "Public status update draft should be prepared.");
  }
  ...
  for (const key of mustRemainFalse) {
    if (config[key] !== false) {
      issue(key, "Vote opening item must remain false until separately authorized.");
    }
  }
  ...
  for (const [key, value] of Object.entries(config.voteOpeningChecklist || {})) {
    if (value !== false) {
      issue(`voteOpeningChecklist.${key}`, "Vote opening checklist item must remain false until actual review.");
    }
  }
...
if (fs.existsSync(governanceVotePath)) {
  ...
  if (vote.fullLaunchApproved !== false) {
    issue("governanceVote.fullLaunchApproved", "Full launch must remain not approved.");
  }
}
...
const result = {
  schema: "astra-full-launch-governance-vote-opening-validation-v0.1",
