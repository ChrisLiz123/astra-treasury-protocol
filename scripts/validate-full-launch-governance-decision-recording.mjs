...
const configPath = path.join(root, "configs", "full-launch-governance-decision-recording.config.json");

const requiredFiles = [
  "docs/full-launch-governance-decision-recording/GOVERNANCE_DECISION_RECORDING_PACKAGE.md",
  "docs/full-launch-governance-decision-recording/GOVERNANCE_DECISION_RECORDING_CHECKLIST.md",
  "docs/full-launch-governance-decision-recording/GOVERNANCE_DECISION_RECORD_TEMPLATE.md",
  "docs/full-launch-governance-decision-recording/GOVERNANCE_DECISION_EVIDENCE_PLAN.md",
  "docs/full-launch-governance-decision-recording/GOVERNANCE_DECISION_DRY_RUN_CASES.md",
  "docs/full-launch-governance-decision-recording/GOVERNANCE_DECISION_RECORDING_BLOCKERS.md",
  "docs/full-launch-governance-decision-recording/GOVERNANCE_DECISION_PUBLIC_STATUS_TEMPLATE.md",
...
if (!fs.existsSync(configPath)) {
  issue("configs/full-launch-governance-decision-recording.config.json", "Missing governance decision recording config.");
}

for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required governance decision recording file.");
  }
}

for (const file of forbiddenDecisionFiles) {
  if (fs.existsSync(path.join(root, file))) {
    issue(file, "Forbidden recorded-decision/approval file exists. This milestone must not record a decision.");
  }
}
...
if (fs.existsSync(configPath)) {
  ...
  if (config.governanceDecisionRecordingPackagePrepared !== true) {
    issue("governanceDecisionRecordingPackagePrepared", "Governance decision recording package should be prepared.");
  }

  if (config.decisionEvidencePlanPrepared !== true) {
    issue("decisionEvidencePlanPrepared", "Decision evidence plan should be prepared.");
  }
  ...
  for (const key of mustRemainFalse) {
    if (config[key] !== false) {
      issue(key, "Decision-recording item must remain false.");
    }
  }
  ...
  if (record.decision !== "NOT RECORDED") {
    issue("draftDecisionRecord.decision", "Draft decision must remain NOT RECORDED.");
  }

  if (Array.isArray(record.approvedCapabilities) && record.approvedCapabilities.length !== 0) {
    issue("draftDecisionRecord.approvedCapabilities", "Approved capabilities must remain empty.");
  }

  if (record.decisionDate) {
    issue("draftDecisionRecord.decisionDate", "Decision date must remain empty.");
  }

  for (const [key, value] of Object.entries(config.requiredBeforeDecisionRecording || {})) {
    if ([
      ...
      if (value !== true) {
        issue(`requiredBeforeDecisionRecording.${key}`, "Preparation prerequisite should be true.");
      }
    } else if (value !== false) {
      issue(`requiredBeforeDecisionRecording.${key}`, "Decision-recording prerequisite must remain false until completed.");
    }
  }

  for (const [key, value] of Object.entries(config.decisionRecordingChecklist || {})) {
    if (value !== false) {
      issue(`decisionRecordingChecklist.${key}`, "Decision recording checklist item must remain false until actual review.");
    }
  }
...
const result = {
  schema: "astra-full-launch-governance-decision-recording-validation-v0.1",
