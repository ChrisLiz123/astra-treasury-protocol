...
const configPath = path.join(root, "configs", "full-launch-governance-vote-authorization.config.json");

const requiredFiles = [
  "docs/full-launch-governance-vote-authorization/GOVERNANCE_VOTE_AUTHORIZATION_PACKAGE.md",
  "docs/full-launch-governance-vote-authorization/GOVERNANCE_VOTE_AUTHORIZATION_CHECKLIST.md",
  "docs/full-launch-governance-vote-authorization/GOVERNANCE_VOTE_AUTHORIZATION_DECISION_RECORD.md",
  "docs/full-launch-governance-vote-authorization/GOVERNANCE_VOTE_AUTHORIZATION_PUBLIC_NOTICE.md",
  "docs/full-launch-governance-vote-authorization/GOVERNANCE_VOTE_AUTHORIZATION_EVIDENCE_PLAN.md",
  "docs/full-launch-governance-vote-authorization/GOVERNANCE_VOTE_AUTHORIZATION_BLOCKERS.md",
...
if (!fs.existsSync(configPath)) {
  issue("configs/full-launch-governance-vote-authorization.config.json", "Missing governance vote authorization config.");
}

for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    issue(file, "Missing required governance vote authorization file.");
  }
}
...
if (fs.existsSync(configPath)) {
  ...
  if (config.governanceVoteAuthorizationPackagePrepared !== true) {
    issue("governanceVoteAuthorizationPackagePrepared", "Governance vote authorization package should be prepared.");
  }

  const mustRemainFalse = [
    "governanceVoteAuthorizationRequested",
    ...
    "fullLaunchApproved",
    "publicVoteNoticeFinalApproved",
    "voteScopeFinalApproved",
    "resolutionTextFinalApproved",
    "capabilityMatrixFinalApproved",
    "voterOrSignerListConfirmed",
    "authorizationEvidencePlanFinalApproved",
    "publicStatusUpdateFinalApproved",
  ...
  for (const key of mustRemainFalse) {
    if (config[key] !== false) {
      issue(key, "Vote authorization item must remain false until separately authorized.");
    }
  }
  ...
  for (const [key, value] of Object.entries(config.requiredBeforeVoteAuthorization || {})) {
    if ([
      "restrictedLaunchStabilized",
      "auditClearanceRecorded",
      "legalClearanceRecorded",
      "fullLaunchGovernanceVotePackageReady",
      ...
      if (value !== true) {
        issue(`requiredBeforeVoteAuthorization.${key}`, "Preparation prerequisite should be true.");
      }
    } else if (value !== false) {
      issue(`requiredBeforeVoteAuthorization.${key}`, "Vote authorization prerequisite must remain false until completed.");
    }
  }
...
const voteOpeningPath = path.join(root, "public-docs", "full-launch-governance-vote-opening-status.json");
if (fs.existsSync(voteOpeningPath)) {
  const opening = JSON.parse(fs.readFileSync(voteOpeningPath, "utf8"));
  ...
  if (opening.governanceVoteOpened !== false) {
    issue("voteOpening.governanceVoteOpened", "Governance vote must remain not opened.");
  }
  if (opening.fullLaunchApproved !== false) {
    issue("voteOpening.fullLaunchApproved", "Full launch must remain not approved.");
  }
}

const governanceVotePath = path.join(root, "public-docs", "full-launch-governance-vote-status.json");
if (fs.existsSync(governanceVotePath)) {
  const vote = JSON.parse(fs.readFileSync(governanceVotePath, "utf8"));
  if (vote.governanceVoteOpened !== false) {
    issue("governanceVote.governanceVoteOpened", "Governance vote must remain not opened.");
  }
...
const result = {
  schema: "astra-full-launch-governance-vote-authorization-validation-v0.1",
